...
- **openai-codex on a ChatGPT account** has entitlement-dependent model access: base GPT includes `gpt-5.5` and `gpt-5.4`, while GPT-5.6 Sol requires a Pro-tier account. Standalone `-codex` variants (`gpt-5.3-codex`, `gpt-5.2-codex`, `gpt-5.1-codex-max` / `-mini`) return `not supported when using Codex with a ChatGPT account`, and a known non-Pro account is rejected locally before a GPT-5.6 Sol turn.
