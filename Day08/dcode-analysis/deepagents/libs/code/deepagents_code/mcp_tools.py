...
# [해설] 모듈 개요: MCP 설정 발견·병합·신뢰 필터·검증 → 서버별 preflight·도구 discovery → LangChain `BaseTool` 변환까지 담당하는 MCP 로더.
# [해설] 실행 프로세스: 주로 서버 프로세스. `server_graph.py`가 그래프 구성 때 `resolve_and_load_mcp_tools(stateless=True, session_manager=...)`를 호출하고,
# [해설] 실제 도구 호출도 서버 프로세스의 캐시 세션(`MCPSessionManager`)으로 나간다. 클라이언트 쪽에서도 `main.py`(신뢰 프롬프트용
# [해설] `discover_mcp_config_sources`·`load_merged_mcp_configs_lenient`·`extract_project_server_summaries`), `tool_catalog.py`(`/tools`),
# [해설] `mcp_login_service.py`(`dcode mcp login` 대상 해석)가 일부 함수를 가져다 쓴다.
# [해설] 주요 진입점: `resolve_and_load_mcp_tools`(전체 파이프라인), `_load_tools_from_config`(preflight+discovery), `MCPSessionManager`,
# [해설] `discover_mcp_config_sources`, `filter_trusted_project_servers`, `load_mcp_config`, `get_mcp_tools`(외부 패키지 호환 API).
# [해설] 관련 모듈: OAuth는 `mcp_auth.py`, `${VAR}` 보간은 `mcp_config.py`, 사용자 disable 목록은 `mcp_disabled.py`, trust list는 `model_config.load_mcp_server_trust_lists`,
# [해설] 플러그인 MCP 레이어는 `plugins/adapters/mcp.py`, readOnlyHint 판정은 `auto_mode.py`(`_deepagents_code_mcp_server` metadata 사용).
# [해설][SDK] deepagents SDK에는 MCP 코드가 없다. 프로토콜·변환은 `langchain_mcp_adapters`, 결과 도구는 `create_deep_agent(tools=...)`의 일반 도구로 들어간다.
# [해설][문서 불일치] SDK 문서(`docs_official/sdk/mcp.md`)는 `langchain.mcp.MCPAdapter`를 안내하지만 dcode는 `langchain_mcp_adapters.sessions/tools`를 쓴다.
# [해설] 관련 문서: `analysis/07-mcp-hooks-extensions-plugins.md`(A절 1~8, 대조표), 공식 `docs_official/code/mcp-tools.md`.
...
# [해설] 도구 이름 규칙 상수: 허용 외 문자는 `_`로 치환, 최대 64자(가장 엄격한 모델 provider 제한), 잘리거나 변형되면 sha256 12자리 접미사.
# [해설] `_MCP_ORIGINAL_TOOL_NAME_KEY`: 도구 metadata에 원래 MCP 도구 이름을 보관하는 키 → `_apply_tool_filter`가 원래 이름으로도 매칭.
...
# [해설] `/mcp` 뷰어·`/tools`에 표시할 도구 1개의 메타데이터(이름·설명·입력 스키마). 실행용 객체가 아니라 표시용 스냅샷.
...
# [해설] 서버 로드 상태 5종. `ok`/`unauthenticated`/`error`는 로더가, `disabled`는 disable 목록 필터가, `awaiting_reconnect`는 TUI가 설정.
# [해설][문서 불일치] 공식 `mcp-tools.md`의 "Server status" 표에는 `disabled`·`awaiting_reconnect`가 없다(grep 확인).
...
# [해설] 서버 1개의 로드 결과. `_load_tools_from_config`/`resolve_and_load_mcp_tools`가 만들어 `server_graph.py`를 거쳐 클라이언트 `/mcp` 뷰어·배너로 전달된다.
# [해설][설계] 실패를 예외 대신 이 레코드로 격리하는 것이 "서버 하나가 실패해도 나머지는 뜬다"의 핵심.
@dataclass(frozen=True, slots=True)
class MCPServerInfo:
    ...
    # [해설] 불변식 검사: ok ↔ error 없음, 비ok ↔ error 필수·tools 없음, pending_reconnect ↔ disabled. 잘못된 조합은 생성 즉시 ValueError.
    ...
    # [해설] 배너의 "unauthenticated 개수" 등 사용자 조치가 필요한 상태 판정.
    def needs_attention(self) -> bool:
        """Return whether this server is blocked on user login."""
        return self.status == "unauthenticated"


# [해설] 원격 전송 종류와 별칭. `streamable_http`/`streamable-http`는 `http`로 정규화(`_resolve_server_type`).
...
# [해설][주의] 서버 이름 검증 정규식 — 이름이 OAuth 토큰 파일명에 쓰이므로 경로 안전성이 걸려 있다. `mcp_auth._SAFE_SERVER_NAME_RE`와 동기화 필요.
_SERVER_NAME_RE = re.compile(r"^[A-Za-z0-9_-]+$")
"""Server names become token-file basenames and must remain path-safe."""


# [해설] MCP 설정 오류 전용 예외. `mcp_auth.login`도 보간 실패를 이 타입으로 바꿔 던지고, `format_login_failure`는 메시지를 그대로 노출한다.
...
# [해설] stdio 서버 stderr 캡처 파라미터: 한 줄 최대 4096자, read 8KiB, 잘림 표시, 디코딩 오류는 replace, drain 스레드 join 대기 2초.
...
# [해설] stdio MCP 서버의 stderr를 받는 파이프 + 드레인 스레드. `_create_mcp_session`이 `stdio_client(errlog=...)`로 넘긴다.
# [해설][설계] 목적 2가지: (1) 서버가 stderr 파이프를 가득 채워 멈추지 않도록 항상 비움, (2) DEBUG 로깅이 켜져 있으면 줄 단위로 정제해 로그.
# [해설] 이 TUI 환경에서는 서버 stderr가 화면을 깨뜨리지 않도록 로그로 돌리는 역할도 한다 (추정).
class _MCPStderrSink(io.TextIOBase):
    ...
    # [해설] 생성 시점에 DEBUG 여부를 고정하고, os.pipe를 만든 뒤 데몬 드레인 스레드를 시작한다. 실패 시 열린 fd를 모두 닫고 재전파.
    ...
    # [해설] 전송 계층이 실제로 쓰는 유일한 API: 자식 프로세스에 넘길 write fd.
    def fileno(self) -> int:
        """Return the write descriptor to hand to the server subprocess."""
        return self._writer.fileno()

    # [해설] 부모 측 write fd 사본을 닫는다. 자식만 write 끝을 가지게 되어 서버 종료 시 EOF가 온다.
    def close(self) -> None:
    ...
    # [해설] 세션 종료 시 드레인 스레드를 최대 2초씩 두 번 기다린다. 파이프를 물려받은 손자 프로세스가 살아 있으면 read fd를 강제로 닫고 포기(데몬 스레드).
    ...
    # [해설] read fd를 정확히 한 번만 닫는다(드레인 스레드와 wait_closed의 경쟁 → fd 번호 재사용으로 무관한 파일을 닫는 사고 방지).
    ...
    # [해설] 드레인 루프: EOF 또는 중단 신호까지 8KiB씩 읽고, 캡처 모드면 디코딩. 드레인이 죽으면 서버가 막힐 수 있으므로 WARNING 이상으로 알린다.
    ...
    # [해설] 증분 디코딩 후 개행 단위로 줄을 방출하고 마지막 조각은 버퍼에 남긴다.
    ...
    # [해설][주의] 유니코드 제어문자(카테고리 C)를 제거해 로그/터미널 제어 시퀀스 주입을 막고, 줄 길이를 상한으로 자른다.
    ...
    # [해설] 현재 줄을 DEBUG 로그로 내보내고 버퍼를 초기화한다.
...
# [해설] MCP 세션 생성 컨텍스트. 원격(http/sse)은 어댑터 `create_session`에 위임하고, stdio만 직접 구성해 stderr를 `_MCPStderrSink`로 돌린다.
# [해설] 호출자: `MCPSessionManager._create_entry`(런타임 캐시 세션), `_load_tools_from_config._discover_server`(1회용 discovery 세션).
# [해설][주의] 어댑터 내부 `_create_stdio_session`을 복제한 것이므로 어댑터 업그레이드 시 동기화가 필요하다.
@asynccontextmanager
async def _create_mcp_session(
    ...
    *,
    ...
    # [해설] stdio: 싱크 생성 → `stdio_client`가 프로세스를 띄운 직후 부모 fd 사본 닫기 → ClientSession yield → finally에서 드레인 종료 대기.
...
# [해설] 세션 전송이 죽었음을 뜻하는 예외인지 판정(anyio 스트림 종료, 파이프/커넥션 리셋, EOF). 도구 호출의 "세션 무효화 후 1회 재시도" 조건.
...
# [해설] 캐시된 세션과 그 정리 스택(AsyncExitStack)의 쌍.
@dataclass(frozen=True, slots=True)
...
# [해설] 연결 설정을 비교 가능한 튜플 서명으로 변환. OAuth provider 객체는 식별성 대신 서버 URL·client metadata·토큰 파일 경로 등으로 비교한다.
# [해설] `MCPSessionManager.configure`가 "활성 세션이 있는데 설정이 바뀌었는가"를 판정하는 데 사용.
...
# [해설] 전체 connections 매핑의 서명(서버 이름 정렬).
...
# [해설] 서버별 지속 세션의 지연 생성 캐시. `server_graph.py`가 프로세스 전역으로 1개 만들어 `resolve_and_load_mcp_tools`에 넘긴다.
# [해설][설계] discovery는 1회용 세션, 실제 도구 호출은 첫 호출 때 만든 세션을 재사용 → stdio 서버를 호출마다 재시작하지 않고,
# [해설] 세션이 그 subprocess/transport를 소유한 이벤트 루프에 묶이도록 한다.
class MCPSessionManager:
    ...
    # [해설] 연결 설정 주입. 세션이 없으면 덮어쓰고, 세션이 있으면 서명이 같을 때만 허용(다른 전송·인증으로 라이브 세션을 재바인딩하는 것을 금지).
    # [해설][주의] 그래프 재빌드 시 MCP 설정이 바뀌었는데 세션이 살아 있으면 RuntimeError → 서버 재시작(`/mcp reconnect`·`/restart`)이 필요한 이유 (추정).
    ...
    # [해설] 캐시 조회 → 없으면 서버별 락 안에서 다시 확인 후 생성(double-checked locking). 동시 첫 호출이 세션을 중복 생성하지 않는다.
    ...
    # [해설] 세션 제거·종료. `expected_session`이 주어지면 그 세션일 때만 제거 — 다른 호출이 이미 새 세션으로 교체한 경우 엉뚱한 세션을 닫지 않게.
    ...
    # [해설] 종료 시 모든 세션을 병렬로 닫는다(서버당 5초 timeout). 이후 생성 요청은 거부. 호출자는 서버 종료 경로 (추정: `server_graph.py`).
    ...
    # [해설] 세션 생성 + `initialize()`(MCP 핸드셰이크, OAuth 흐름도 이때 발생). 실패 시 같은 태스크에서 스택을 닫아 anyio cancel scope 오류를 피한다.
...
# [해설] 전송 결정: `type` → `transport` → url 있으면 http → 없으면 stdio. Claude Code `.mcp.json`의 `{"url": ...}` 관례와 호환.
...
# [해설] 서버 설정 1개의 형태 검증(보간 전). `resolve_and_load_mcp_tools`, `load_mcp_config`, `_drop_invalid_mcp_config_servers`가 호출.
# [해설][흐름] 1) 이름 regex → 2) dict 여부 → 3) 전송별 필수 필드·상호 배타(command vs url)·타입 → 4) `auth`는 "oauth"만, 원격 전용, Authorization 헤더와 병용 금지 → 5) 도구 필터 필드.
# [해설][설계] `${VAR}` 보간은 활성화 시점(`_preflight_and_connect`)으로 미뤄, 환경변수 하나가 없어도 그 서버만 실패하게 한다.
def _validate_server_config(server_name: str, server_config: dict[str, Any]) -> None:
    ...
    # [해설][흐름] 4) OAuth 제약: 정적 Authorization 헤더와 OAuth를 함께 쓰면 어느 자격증명이 쓰일지 모호하므로 거부.
...
# [해설] `allowedTools`/`disabledTools` 검증: 둘 중 하나만, 비어 있지 않은 문자열 리스트여야 한다(빈 리스트는 사용자 실수로 간주해 거부).
...
# [해설] 이하 4개 함수(`_looks_like_comment`, `_json_error_hint`, `_trailing_comma_pos`, `_json_error_snippet`)는 JSON 파싱 오류를
# [해설] "힌트 + 캐럿 스니펫"이 붙은 사용자 친화 메시지로 만드는 헬퍼. `_load_mcp_config_json`만 사용.
...
# [해설] `.mcp.json` 파일 로드. JSONDecodeError는 힌트·스니펫을 붙인 새 JSONDecodeError로 다시 던진다(위치 정보 유지).
...
# [해설] 최상위 형태 검증: `mcpServers` 키 필수, dict, 비어 있지 않음.
...
# [해설] 모든 서버를 검증 — 하나라도 틀리면 예외(파일 전체 거부). 대조: `_drop_invalid_mcp_config_servers`는 틀린 서버만 제외.
...
# [해설] 우선순위 해석 "후"에 틀린 서버만 뺀다. 이긴(상위) 정의가 틀렸다고 가려졌던 하위 정의가 되살아나지 않게 하기 위함.
...
# [해설] 파일 로드 + 최상위 형태만 검증(서버별 검증은 병합 후로 미룸). 프로젝트 config 경로가 사용.
...
# [해설] 공개 API: 파일 로드 + 전체 검증(엄격). user config와 `--mcp-config`(explicit) 경로, `get_mcp_tools`가 사용.
...
# [해설] 프로젝트 MCP 설정 탐색 기준 디렉터리: 프로젝트 루트(git 등) → 없으면 사용자 cwd.
...
# [해설] 프로젝트(및 플러그인) 서버 신뢰 규칙의 단일 원천. `resolve_and_load_mcp_tools`와 `mcp_login_service.py`가 공유.
# [해설] 규칙: disabled 이름은 무조건 제외(거부 우선) → config 전체 신뢰면 유지 → 아니면 `trust_lists.is_enabled`(project_root+fingerprint 승인 또는 env 허용목록).
# [해설][주의] 보안 경계: 저장소에 커밋된 `.mcp.json`은 stdio 명령 실행·원격 SSRF/헤더 보간 유출이 가능하므로 기본 거부(default-deny).
...
# [해설] 도움말·오류 메시지 표시용 발견 경로 목록(낮은 → 높은 우선순위). 실제 발견은 `discover_mcp_config_sources`가 수행.
...
# [해설] 발견된 config의 신뢰 출처: USER(사용자 프로필, 암묵 신뢰) / PROJECT(저장소 소유, 승인 필요).
...
# [해설] 경로 + scope + (PROJECT면 필수인) project_root. project_root는 승인 기록의 키이므로 누락을 생성 시점에 거부.
@dataclass(frozen=True, slots=True)
...
# [해설] 두 경로가 같은 파일인지(SAME/DIFFERENT/UNKNOWN). UNKNOWN은 fail-closed 판단에 쓰인다.
...
# [해설] 문자열 비교 → `samefile`(symlink·대소문자 별칭 처리) → 실패 시 UNKNOWN.
...
# [해설] 후보 추가 시 충돌 처리. 핵심 보안 규칙: 충돌은 절대 USER 신뢰를 부여하지 않고 PROJECT 쪽으로 강등한다.
# [해설] 예) DEEPAGENTS_HOME을 프로젝트 안으로 옮겨 user config와 프로젝트 config가 같은 파일이 되면 그 서버들은 프로젝트 승인이 필요해진다.
def _append_discovered_config(
    ...
    if candidate.scope is not MCPConfigScope.PROJECT:
        ...
        return
    # [해설] 같은 파일 → 기존 항목을 빼고 현재(더 높은 우선순위) 위치로 이동 / 판별 불가 → 기존 항목을 PROJECT로 강등해 둘 다 유지.
...
# [해설] MCP 설정 자동 발견. 순서(낮음→높음): user `~/.deepagents/.mcp.json` → `<project>/.deepagents/.mcp.json` → `<project>/.mcp.json`.
# [해설] 존재하는 파일만 scope와 함께 반환. `resolve_and_load_mcp_tools`, `main.py`(신뢰 프롬프트), `mcp_login_service.py`가 호출.
...
# [해설] 발견 결과를 scope별로 분할한 뷰. 경로 모양으로 신뢰를 추론하던 과거 방식을 대체해 출처를 한 곳에서만 결정한다.
@dataclass(frozen=True, slots=True)
...
# [해설] stdio 서버의 (이름, command, args) 목록 추출. 호출처는 이 파일 밖(추정: 신뢰 프롬프트/경고 표시용).
...
# [해설] 신뢰 프롬프트에 보여줄 프로젝트 서버 한 줄(이름, 전송, 명령 또는 URL). 검증 전 원시 설정에서 만든다.
...
# [해설] 프로젝트 서버 요약 목록. 신뢰 프롬프트(`main.py`)와 skip 경고가 사용 — stdio와 원격을 동일하게 게이트하기 위해 둘 다 포함.
...
# [해설] 서버 이름 기준 얕은 병합: 뒤(높은 우선순위)가 같은 이름을 객체 통째로 덮어쓴다(deep merge 없음 — 공식 문서와 일치).
...
# [해설] 병합하면서 각 서버의 "이긴" 파일 경로를 기록. 프로젝트 서버별 project_root를 찾는 데 사용.
...
# [해설] 단일 파일 관대 로더(오류 시 None). docstring상 현재 프로덕션 호출처 없음.
...
# [해설] 프로젝트 config들을 병합·disabled 제거·틀린 서버 제외한 결과. 신뢰 프롬프트(`main.py`)가 "승인하면 실제로 활성화될 정의"를 보여주는 데 사용.
...
# [해설] (config, error) 반환형 로더. 파일 없음은 오류가 아님. user config 로드에 사용되며 오류는 `<config:...>` 행으로 표면화된다.
...
# [해설] 최상위 형태만 검증하는 (config, error) 로더. 프로젝트 config 로드에 사용.
...
# [해설] stdio preflight: command가 PATH에 있는지만 확인(실행하지 않음). `shutil.which`가 블로킹이라 호출자가 `to_thread`로 돌린다.
...
# [해설] 원격 preflight: 2초 timeout HEAD 요청. 네트워크 오류 또는 5xx만 실패로 본다(401/404 등은 통과 → 이후 discovery에서 OAuth 챌린지로 분류).
# [해설][주의] 오류 메시지에 URL을 넣지 않고 예외 클래스명만 남긴다 — URL에 보간된 비밀이 있을 수 있기 때문.
...
# [해설] 원시 설정에 `${...}`가 있는지. 있으면 이후 실패 메시지·로그에서 상세(보간된 값이 들어 있을 수 있음)를 가린다.
...
# [해설] `list_tools` 페이지네이션. 커서가 끝나지 않는 서버를 대비해 1000페이지 상한.
...
# [해설] 모델이 선택 문자열 인자에 `""`를 채워 보내면 생략으로 간주해 제거(Slack MCP 등이 빈 ID를 거부하는 문제 대응).
# [해설] 필수 필드와 명시적 비문자열 타입 필드는 유지. 캐시 세션 도구의 `coroutine`에서만 적용된다(stateless 어댑터 도구 경로는 미적용).
...
# [해설] LangChain 도구 이름 = `{server}_{tool}`. 문자가 치환되거나 64자를 넘으면 앞부분을 잘라 `_<sha256 12자리>`를 붙여 결정적·고유하게 만든다.
# [해설] 이 이름이 HITL/Auto 모드·`allowedTools` 매칭·훅 wire 이름 변환의 입력이 된다. 공식 문서는 `{server}_{tool}`만 언급.
...
# [해설] `MCPSessionManager` 캐시 세션을 쓰는 `StructuredTool` 생성(서버 모드 경로). `_discover_server`가 호출.
# [해설][흐름] 1) 이름·metadata 구성 → 2) 호출 시 인자 정규화 → 세션 획득 → `call_tool` → 3) 재인증 필요면 세션 무효화 후 ToolException
# [해설][흐름] 4) 전송 사망이면 무효화 후 새 세션으로 1회 재시도 → 5) 결과를 어댑터 `_convert_call_tool_result`로 content+artifact 변환.
def _build_cached_mcp_tool(
    *,
    ...
    # [해설] metadata: MCP `annotations`(readOnlyHint/destructiveHint 등)를 펼쳐 넣고, `_meta`, dcode 표식(`_deepagents_code_mcp`, 서버명, 원래 이름)을 추가.
    # [해설] `auto_mode.py`의 `mcp_tool_is_coherently_read_only`가 이 hint들로 분류기 우회 여부를 판단한다.
    ...
    # [해설] 도구 로컬 오류 핸들러: MCP `isError` 결과 등 복구 가능한 ToolException을 모델이 볼 error ToolMessage 텍스트로 바꾸고 1회만 로그.
    ...
    # [해설] 실제 도구 호출 코루틴(서버 프로세스의 그래프 이벤트 루프에서 실행).
    async def coroutine(
        ...
        **arguments: Any,
        ...
        # handler and the model.
            ...
            raise
        # [해설][흐름] 3) 재인증 필요(`MCPReauthRequiredError`, 비대화형 OAuth provider가 던짐) → 세션을 버리고 `/mcp login` 안내를 도구 오류로 모델에 반환.
        # [해설] 전송 사망이 아닌 일반 예외 → 재시도 없이 ToolException.
            ...
            # [해설][흐름] 4) 전송 사망: 캐시 세션을 무효화하고 새 세션으로 딱 1번 재시도. 재시도 실패 시에도 재시도 세션을 정리한다.
            logger.info(
    ...
    # [해설] `response_format="content_and_artifact"`: 어댑터 변환 결과(텍스트 content + 비텍스트 artifact)를 ToolMessage에 나눠 담는다.
...
# [해설] 필터 항목이 glob인지 판단하는 문자 집합.
_GLOB_METACHARS = frozenset("*?[")


# [해설] 필터 항목 1개 매칭: 후보 = LangChain 이름, (있으면) 원래 MCP 이름, `{server}_{원래 이름}`. glob이면 fnmatchcase(대소문자 구분), 아니면 정확 일치.
# [해설] 원래 이름을 metadata에서 가져오므로 해시가 붙어 잘린 이름이라도 사용자가 쓴 이름으로 매칭된다.
...
# [해설] 입력 타입(list/Sequence)을 보존하는 오버로드 선언.
@overload
...
# [해설] 서버별 `allowedTools`(화이트리스트) 또는 `disabledTools`(블랙리스트) 적용. 아무 도구와도 맞지 않는 항목은 오류 대신 경고(서버 버전 차이 허용).
def _apply_tool_filter(
    tools: Sequence[BaseTool],
    server_name: str,
    server_config: dict[str, Any],
...
# [해설] 서버 preflight·discovery 동시 실행 상한 8(문서 미기재). 큰 설정이 소켓·subprocess 핸드셰이크를 무제한 띄우지 않게 한다.
...
# [해설] blockbuster(서버 이벤트 루프의 블로킹 IO 감지기)를 피하려고 첫 import 시 디스크 IO가 있는 모듈을 스레드에서 미리 import.
...
# [해설] 세마포어로 동시성을 제한하면서 제출 순서대로 결과를 반환하는 gather. 하나라도 실패하면 나머지를 취소·회수하고 첫 예외를 전파.
...
# [해설] 검증된 병합 설정으로 연결 구성 → 도구 로드. `resolve_and_load_mcp_tools`와 `get_mcp_tools`가 호출.
# [해설][흐름] 1) 어댑터 import 워밍 → 2) 서버별 `_preflight_and_connect`를 동시성 8로(보간·preflight·Connection+OAuth 구성)
# [해설][흐름] 3) 세션 매니저 결정 → 4) 서버별 `_discover_server`를 동시성 8로(1회용 세션 discovery, 실패 분류, 도구 생성·필터)
# [해설][흐름] 5) 설정 순서대로 `MCPServerInfo` 조립, 도구는 이름순 정렬해 반환.
# [해설] 도구 모드 3가지: 외부 매니저 주입(서버 모드: 캐시 세션 도구) / 매니저 없음+stateful(새 매니저 생성·반환) / stateless+매니저 없음(호출마다 새 세션인 어댑터 도구).
async def _load_tools_from_config(
    ...
    *,
    ...
    # [해설][흐름] 1) 서버 목록·전송 종류를 한 번만 계산. `oauth_servers`는 preflight에서 OAuth provider를 붙인 서버 집합(→ `uses_oauth`).
    ...
    # [해설][흐름] 2) 서버 1개 preflight + Connection 구성. 실패는 `(status, error)` 튜플로 반환(예외 전파 안 함).
    async def _preflight_and_connect(
        ...
        # [해설] 2-a) `${VAR}` / `${VAR:-default}` 보간(`mcp_config.resolve_mcp_server_env`). 여기서 실패하면 이 서버만 error.
        try:
        ...
        # [해설] 2-b) 연결성 preflight: 원격은 HEAD, stdio는 PATH 확인.
        try:
            if server_type in _SUPPORTED_REMOTE_TYPES:
        ...
        # [해설] 2-c) Connection 구성. 원격은 StreamableHttp/SSE + 헤더 + (조건부) OAuth provider, stdio는 command/args/env.
        try:
            if server_type in _SUPPORTED_REMOTE_TYPES:
                if server_type == "http":
                ...
                # [해설] OAuth 결정 규칙: `auth: oauth`인데 저장 토큰 없음 → 연결 시도 없이 `unauthenticated`.
                # [해설] `auth: oauth`이거나, (선언 없어도) 이전 로그인으로 저장된 토큰이 있고 정적 Authorization 헤더가 없으면 → 비대화형 provider 부착.
                ...
                if explicit_oauth or (
                    ...
                    # [해설] 서버 프로세스이므로 `interactive=False` — 브라우저를 띄울 수 없고, 토큰 만료·refresh 실패는 `MCPReauthRequiredError`로 표면화된다.
        ...
        # [해설] 설정/구성 실패: 보간을 쓴 설정이면 상세 메시지를 가려서 기록(비밀 유출 방지).
    ...
    # [해설][흐름] 2) 병렬 preflight 실행 후 skipped / connections로 분리(설정 순서 유지).
    ...
    # [해설][흐름] 3) 세션 매니저: 주입된 매니저는 `configure`로 연결 갱신(활성 세션과 설정이 다르면 RuntimeError), 없고 stateful이면 새로 만든다.
    ...
    # [해설][흐름] 4) 서버 1개 discovery + 도구 구성. 모든 실패를 `MCPServerInfo`로 격리, 취소·종료 신호만 재전파.
    async def _discover_server(
        ...
        # [해설] 4-a) 1회용 세션으로 initialize + list_tools. 원격 OAuth 서버는 여기서 토큰 refresh/재인증 판정이 처음 일어난다.
        try:
            ...
            # [해설] 4-b) 실패 분류 1: 저장 토큰이 있었지만 refresh 실패 → `unauthenticated`.
            if reauth is not None:
            ...
            # [해설] 4-b) 실패 분류 2: OAuth 미선언 서버가 401 + RFC 9728 Bearer 챌린지 → `unauthenticated`로 자동 감지(로그인 안내).
        ...
        # [해설] 4-c) 도구 구성: 매니저가 없으면 어댑터 `convert_mcp_tool_to_langchain_tool`(호출마다 새 세션)로 만들고 이름·metadata를 dcode 규칙으로 덮어쓴다.
        # [해설] 매니저가 있으면 캐시 세션 기반 `_build_cached_mcp_tool`. server_graph 경로(stateless=True + 매니저 주입)는 후자다.
        try:
            ...
            # [해설] 4-d) allowed/disabled 필터 적용 후 표시용 스키마를 LangChain 이름 기준으로 짝지어 `MCPToolInfo` 생성(deepcopy로 공유 가변 객체 방지).
    ...
    # [해설][흐름] 5) preflight를 통과한 서버만 병렬 discovery → 설정 순서로 skipped와 합쳐 `server_infos` 구성.
    ...
    # [해설] 도구는 이름순 정렬(결정적 순서 → 프롬프트 캐시 안정성에도 유리 (추정)). stateless면 매니저는 반환하지 않는다(소유권은 호출자).
    all_tools.sort(key=lambda tool: tool.name)
    return all_tools, None if stateless else runtime_manager, server_infos


# [해설] 단일 파일 기반 공개 API(`deepagents-talon` 호환). 신뢰·발견·disable 필터 없이 바로 로드한다.
...
# [해설] 신뢰 결정으로 제외된 프로젝트 서버를 이유별 문구로 경고(명시 거부 / `--trust-project-mcp` 미사용 / 설정 변경 또는 미승인).
...
# [해설] trust list 읽기 오류·레거시 설정(`enabled_project_servers`, 옛 env 이름)·손상된 승인 항목을 `<config:...>` 오류 행으로 표면화할 알림 목록.
# [해설] 비대화형 서버 경로에서는 logger.warning이 보이지 않으므로 뷰어에 보이는 오류로 바꾼다.
...
# [해설] MCP 전체 파이프라인 진입점. 서버 프로세스 `server_graph.py`(stateless=True, 전역 세션 매니저, 플러그인 레이어 전달), `main.py`, `tool_catalog.py`가 호출.
# [해설][흐름] 0) `--no-mcp`면 즉시 빈 결과 → 1) 설정 발견 → 2) user config 로드(암묵 신뢰) → 3) trust list 로드(플러그인/프로젝트 있을 때만)
# [해설][흐름] 4) 플러그인 레이어: deny만 적용, 정책 읽기 실패 시 fail-closed → 5) 프로젝트: 병합(우선순위) 먼저, 서버별 신뢰 필터 나중
# [해설][흐름] 6) `--mcp-config`(최상위, 오류 치명적) → 7) 전체 병합 후 managed/user disabled 필터 → 8) 전체 검증 → `_load_tools_from_config`.
# [해설] 병합 우선순위(낮음→높음): user → plugin → project → explicit (configs 리스트에 append되는 순서).
async def resolve_and_load_mcp_tools(
    *,
    ...
    # [해설][흐름] 1) 자동 발견. 실패는 `<discovery>` 오류 행으로 남기고 계속.
    try:
    ...
    # [해설][흐름] 2) user config: 사용자 프로필 소유이므로 신뢰 필터 없이 추가. 파일 오류는 오류 행.
    ...
    # [해설][흐름] 3) trust list는 사용자 홈 config.toml + env에서만 읽는다 — 저장소의 `.mcp.json`이 스스로를 승인할 수 없다.
    ...
    # [해설][흐름] 4) 플러그인 MCP: 설치 = 동의. `config_trusted=not load_failed`로 disabled만 거르고, 정책을 못 읽으면 전부 제외(fail-closed).
    # [해설] 서버 이름은 이미 `plugin__<id>__<server>`로 네임스페이스된 상태(`plugins/adapters/mcp.py`의 `scoped_mcp_server_name`).
    # [해설][주의] 플러그인 서버는 여기서 형태 검증을 하지 않는다. 틀린 정의는 8) 전체 검증에서 RuntimeError로 전체 로드를 실패시킬 수 있다 (추정, 미검증).
    ...
    # [해설][흐름] 5) 프로젝트 config: 최상위 형태만 검증해 로드(서버별 검증은 신뢰 결정 뒤).
    ...
    if loaded_project_configs and trust_lists is not None:
        ...
        # [해설] config 전체 신뢰는 `trust_project_mcp is True`(플래그 또는 인터랙티브 승인)일 때만. None은 False와 같다.
        ...
        # currently skips).
        # [해설] 서버마다 "이긴 정의가 나온 파일"의 project_root로 승인(fingerprint)을 대조. 상위 파일이 승인된 서버 정의를 바꾸면 거부되고,
        # [해설] 가려진 옛 승인 정의가 되살아나지 않는다.
        kept: dict[str, Any] = {}
        ...
        # [해설] 신뢰 통과 서버 중 형태가 틀린 것만 오류 행으로 제외. 아무것도 신뢰되지 않았고 요약도 없으면(전부 기형) 정확한 오류를 만들기 위해 재검증.
        if kept:
    ...
    # [해설][흐름] 6) `--mcp-config`: 사용자가 명시한 파일이라 신뢰, 최상위 우선순위. `load_mcp_config` 예외가 그대로 전파(치명적).
    ...
    # [해설] 누적된 설정 오류를 `<config:파일명>` 이름의 error 상태 `MCPServerInfo`로 변환(뷰어에 표시).
    ...
    # [해설][흐름] 7) disable 필터: managed 정책 + 사용자 `/mcp` F2 토글 목록의 합집합(`mcp_disabled.get_disabled_servers`).
    # [해설][주의] managed 정책을 읽지 못하면 관리자가 막았을 수 있으므로 모든 서버를 비활성화(fail-closed, 문서 미기재).
    ...
    # [해설][흐름] 8) 최종 전체 검증(실패 시 RuntimeError) → 로드 → 서버 정보 + disabled 행 + 설정 오류 행을 합쳐 반환.
    try:
        for server_name, server_config in merged["mcpServers"].items():
            _validate_server_config(server_name, server_config)
