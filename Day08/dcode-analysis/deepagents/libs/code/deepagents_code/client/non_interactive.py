...
"""
# [해설] ── 모듈 개요 ─────────────────────────────────────────────
# [해설] 역할: 헤드리스(비대화형) 1회 실행. `dcode -n "..."` 또는 파이프 stdin 입력을 서버 에이전트에 보내고,
# [해설]   스트림(messages/updates/custom)을 stdout/stderr로 렌더링하며, HITL 인터럽트를 정책에 따라 자동 결정한 뒤 exit code로 끝낸다.
# [해설] 실행 위치: **클라이언트 프로세스**. 에이전트 그래프는 `server_manager.server_session`이 띄운 langgraph dev 서버에서 돈다.
# [해설] 주요 진입점: `run_non_interactive`(main.py `cli_main`이 호출) → `_run_agent_loop` → `_stream_agent` → `_process_stream_chunk`.
# [해설] 부가 책임: Hooks v2 클라이언트 측 실행(tool.use/tool.result/session.* 이벤트, hook 인터럽트 이행), 사용량·비용 집계,
# [해설]   모델 재시도(model_attempt/model_retry) 조정, 서브에이전트 transcript 기록, `--startup-cmd` 실행.
# [해설] 관련 분석: analysis/01-boot-client-server.md (흐름 C), analysis/04-approval-hitl-security.md, analysis/07-mcp-hooks-extensions-plugins.md
# [해설] 관련 문서: docs_official/code/cli-reference.md(-n, -q, --no-stream, --max-turns, --timeout, -S), docs_official/code/quickstart.md
# [해설] exit code 체계(요약): 0 성공/hook stop, 1 오류, 124 HITL 반복 한도(및 main.py의 --timeout), 130 사용자 인터럽트.
...
# [해설] HITL 인터럽트 왕복이 `_MAX_HITL_ITERATIONS`(50)를 넘을 때의 예외. `run_non_interactive`가 exit 124로 매핑.
class HITLIterationLimitError(RuntimeError):
    """Raised when the HITL interrupt loop exceeds `_MAX_HITL_ITERATIONS` rounds."""


# [해설] 스트림 제어 try 블록 밖에서 raise하기 위한 헬퍼(NoReturn). 예외 분류가 스트림 오류 처리에 삼켜지지 않게 한다(추정).
...
# [해설] 클라이언트 hook이 세션 중단을 요구할 때 `ClientHookStopError`를 던진다. `run_non_interactive`에서 exit 0으로 처리.
...
# [해설] 서버에서 온 `__interrupt__` 값을 langchain `HITLRequest` TypedDict로 검증하는 pydantic 어댑터(`_process_interrupts`에서 사용).
_HITL_REQUEST_ADAPTER = TypeAdapter(HITLRequest)

# [해설] 스트림 청크 형식 상수: subgraphs=True 스트리밍이라 (namespace, mode, data) 3-튜플, messages 모드 data는 (메시지, 메타데이터).
...
# [해설] 에이전트가 거절된 명령을 계속 재시도하는 무한 루프 방지용 HITL 왕복 상한.
...
# [해설] 진행 중인 모델 호출 시도 1건의 클라이언트 측 식별(call_id, attempt 번호, transcript agent_id).
# [해설] 서버 모델 재시도 미들웨어가 custom 스트림으로 보내는 `model_attempt` 이벤트로 열리고 닫힌다(`_process_attempt_lifecycle`).
...
# [해설] 응답 텍스트 출력은 Rich Console이 아닌 sys.stdout 직접 사용 → quiet 모드(콘솔=stderr)에서도 stdout에는 답변만 남는다.
...
# [해설] 레거시 Windows 코드페이지에서 장식 글리프 출력이 UnicodeEncodeError로 실행 전체를 죽이지 않도록 errors="replace"로 재설정.
# [해설] 호출: `run_non_interactive` 시작부.
...
# [해설] verbose(비 quiet) 모드의 Rich Live 스피너. 텍스트/도구 출력 직전에 stop, 대기 구간에 start를 반복한다.
...
# [해설] `--startup-cmd` 셸 서브프로세스 종료·수거. POSIX는 프로세스 그룹 SIGTERM → 5초 → SIGKILL, Windows는 kill.
# [해설] 호출: `_run_startup_command`의 타임아웃/취소 경로.
...
# [해설] tool.use hook이 발화됐지만 결과가 아직 오지 않은 도구 호출(이름+인자 묶음). 중단 시 고아 정리에 쓰인다.
...
# [해설] hooks 설정 전 사용하는 "아무 hook도 실행하지 않는" HooksManager. StreamState 기본값.
...
# [해설] 스피너가 준비되기 전 로드된 hook의 알림을 스타일 없이 출력하는 싱크.
...
# [해설] 스트림 한 번(및 HITL 재개 패스들)에 걸쳐 누적되는 가변 상태 묶음.
# [해설] 크게 ① 출력 제어(quiet/stream/reasoning) ② 도구 호출 버퍼·hook 상관관계 ③ HITL/hook 인터럽트 대기·응답
# [해설]   ④ 사용량 원장(stats, recorded_usage_requests) ⑤ 모델 시도 수명주기(active_attempts 등) ⑥ transcript 식별로 나뉜다.
# [해설] 생성: `_run_agent_loop`, 소비: `_process_stream_chunk` 계열 함수 전부.
@dataclass
class StreamState:
...
# [해설] LangSmith 스레드 URL을 백그라운드 스레드에서 조회한 결과 컨테이너(실행 종료 시 링크 출력용).
@dataclass
...
# [해설] 데몬 스레드로 `build_langsmith_thread_url`을 호출(네트워크 가능)해 메인 흐름을 막지 않는다.
...
# [해설] `updates` 청크의 `__interrupt__`를 분류해 상태에 기록.
# [해설] - Hooks v2 인터럽트 → pending_hook_interrupts(클라이언트에서 hook 실행 후 재개)
# [해설] - 정상 HITLRequest → pending_interrupts(이후 `_process_hitl_interrupts`가 hook·자동 정책으로 결정)
# [해설] - 형식 오류 → 즉시 reject 결정(fail-closed)
# [해설][SDK] HITL 인터럽트 형식은 langchain `HumanInTheLoopMiddleware`(SDK create_deep_agent가 interrupt_on으로 구성).
def _process_interrupts(
    ...
    if interrupts:
        for interrupt_obj in interrupts:
            ...
            state.interrupt_occurred = True
            # [해설] NOTIFICATION hook 핸들러가 없을 때만 레거시 `input.required` 이벤트를 fire-and-forget으로 보낸다.
            if not state.hooks.has_handlers(HookEvent.NOTIFICATION):
                dispatch_hook_fire_and_forget("input.required", {})


# [해설] 스트리밍된 AIMessage의 usage를 SessionStats 원장에 기록(메인/서브에이전트 구분, 시도 범위로 중복 방지).
...
# [해설] `--show-reasoning` 시 모델 reasoning을 stderr에 `Reasoning:` 머리말과 함께 출력.
...
# [해설] reasoning 구간을 줄바꿈으로 닫는다(텍스트·도구 출력 전).
...
# [해설] AIMessage content_blocks를 순회하며 text(stdout 또는 버퍼), reasoning(stderr), tool_call(_chunk)(버퍼 누적·표시·hook)을 처리.
def _process_ai_message(
    ...
    for block in message_obj.content_blocks:
        ...
        # [해설][흐름] 도구 호출 청크: 1) 버퍼 키(index/id) 결정·누적 → 2) 이름이 확정되면 "Calling tool" 1회 표시
        # [해설]   → 3) id·인자가 파싱되면 tool.use hook 1회 발화 + in-flight 등록 → 4) 버퍼 제거(인덱스 재사용 대비).
            ...
            if (
                ...
                # [해설] `--no-stream`에서는 상태 줄을 즉시 찍지 않고 시도 완료 시까지 스테이징(실패 시 철회 가능하게).
...
# [해설] messages 모드 청크 처리: 사용량 기록 → summarization 합성 메시지 숨김 → AIMessage/ToolMessage 분기.
# [해설] ToolMessage 분기: in-flight 상관관계 해제 → 파일 변경 diff/경고 표시 → tool.error/tool.result hook 발화(항상).
def _process_message_chunk(
    ...
    # [해설] lc_source == "summarization": SDK 요약(compaction) 미들웨어가 넣은 내부 메시지. 렌더링하지 않고 관찰 플래그만 세운다.
    # [해설][SDK] 요약 미들웨어는 SDK `deepagents` 쪽 SummarizationMiddleware 계열(추정). 관련: analysis/02.
    ...
    if isinstance(message_obj, AIMessage):
        ...
        if isinstance(tool_id, str):
            ...
            tool_args = {}
        # [해설] FileOpTracker가 도구 결과를 파일 조작 기록으로 완성하고, 검증 불가 등의 경고(caveat)를 계산한다.
        ...
        # the full guarantee.
        # [해설] 상태 정규화 후 오류면 tool.error를 먼저, 그다음 항상 tool.result를 보낸다(hook 소비자에게 terminal 이벤트 보장).
...
# [해설] custom 스트림의 RubricMiddleware 이벤트(acceptance criteria 평가 시작/종료)를 콘솔에 표시.
# [해설][SDK] RubricMiddleware는 SDK 쪽 미들웨어가 `runtime.stream_writer`로 방출. 관련 분석: analysis/05-subagents-goals-rubrics.md
def _process_rubric_event(
    ...
    # [해설] iteration은 신뢰할 수 없는 스트림 페이로드이므로 int일 때만 표시(과거 TypeError로 실행이 중단된 버그 방지).
    ...
    # [해설] 결과별 표시: satisfied / needs_revision(실패 기준 나열) / max_iterations_reached / failed·grader_error / 미지의 결과.
...
# [해설] 시도 식별 3-튜플(namespace, call_id, attempt). 사용량 범위 키와 버퍼 오프셋 키로 쓰인다.
def _attempt_key(
...
# [해설] 루트 네임스페이스(())에 열린 시도가 있으면 그 키, 없으면 None(레거시 비범위 기록).
...
# [해설] 수명주기 이벤트의 transcript 스테이징 대상 결정: 루트=None, 서브에이전트=관측된 agent_id 또는 네임스페이스 라벨.
...
# [해설] 서브에이전트 시도가 agent_id를 모르는 채 열렸다가 첫 메시지로 실제 ID를 알게 되면, 스테이징을 실제 ID로 옮긴다.
...
# [해설] 실패(대체된) 모델 시도가 남긴 흔적을 되돌린다. 호출: `_process_retry_lifecycle`, 그리고 retry 이벤트 유실 시 `_process_attempt_lifecycle`.
# [해설] 루트일 때: 미실행 도구 hook 정리 → 도구 버퍼 비움 → 출력 조정(--no-stream은 버퍼 잘라내기, 스트리밍은 경계선 출력).
def _reconcile_superseded_attempt(
    ...
    *,
    ...
    """
    # [해설][흐름] 1) 멱등 처리: 이미 정리한 시도면 무시.
    if attempt_id in state.settled_attempts:
        return
    state.settled_attempts.add(attempt_id)

    # [해설][흐름] 2) 시도 범위 닫기: active_attempts/오프셋 제거, transcript 스테이징 폐기. 서브에이전트면 여기서 끝.
    ...
    # [해설][흐름] 3) (루트) 실패 시도의 in-flight 도구에 합성 terminal 이벤트 발화, 버퍼 정리.
    ...
    # [해설][흐름] 4) 출력 조정. 이미 stdout에 쓴 텍스트는 되돌릴 수 없으므로 RETRY_BOUNDARY_LINE으로 경계를 표시.
    if not state.stream:
...
# [해설] --no-stream 모드에서 성공한 시도의 스테이징된 도구 상태 줄을 출력.
...
# [해설] 성공한 시도 확정: 범위 제거, transcript 확정, (루트면) 스테이징된 도구 상태 줄 출력.
...
# [해설] `model_attempt` custom 이벤트 처리(start/complete). 출력은 없고 범위·오프셋·transcript 스테이징만 관리한다.
# [해설] 서버 쪽 방출: `model_retry.py` 계열 미들웨어(추정). 관련 분석: analysis/02(모델 재시도).
def _process_attempt_lifecycle(
    ...
    # [해설][흐름] start: 기존 열린 시도가 있으면 — 다른 call이면 완료 이벤트 유실로 보고 커밋, 같은 call이면 retry 유실로 보고 정리.
    if data["phase"] == "start":
        scope = _AttemptLifecycleScope(
            call_id=call_id,
            attempt=attempt,
    ...
    # [해설][흐름] complete: 현재 열린 시도와 정확히 일치할 때만 커밋(순서 어긋남·중복 이벤트는 무시).
    scope = state.active_attempts.get(namespace)
...
# [해설] 재시도로 대체된 시도에서 발화된 tool.use에 합성 tool.error/tool.result를 보내 "use 1회 = terminal 1회" 계약을 지킨다.
# [해설] 정리된 id는 단조 집합에서 빼서, 재실행 시 같은 id가 다시 tool.use를 발화할 수 있게 한다.
...
# [해설] 상관관계가 검증된 `model_retry` 이벤트 처리. 대체 시도 정리 후(루트면) 재시도 상태 줄(백오프 안내)을 출력.
...
# [해설] 원시 스트림 청크 라우터(핫 루프). 호출: `_stream_agent`.
# [해설][흐름] 1) 형식 검사 → 2) transcript 기록(모든 네임스페이스) → 3) 서브에이전트: custom 이벤트(시도/재시도/사용량)만 처리
# [해설]   → 4) 루트: messages/updates/custom 모드별 핸들러로 분기.
def _process_stream_chunk(
    ...
    # [해설][흐름] 2) transcript 기록 + 서브에이전트 네임스페이스의 transcript agent_id 학습(첫 메시지에서만).
    if (
    ...
    # [해설][흐름] 3) 서브에이전트 청크: 화면 렌더링은 생략하지만 비용·사용량과 시도 수명주기는 반영.
    ...
    # [해설][흐름] 4) 루트 청크 분기: updates의 __interrupt__ → 인터럽트 기록 / custom → 재시도·시도·rubric 이벤트 / messages → 메시지 렌더링.
    if stream_mode == "updates" and isinstance(data, dict) and "__interrupt__" in data:
        ...
        if data.get("type") == "model_retry":
            ...
            if correlation is not None:
                ...
                # [해설][주의] 레거시(상관관계 없는) model_retry: 출력이 시작되기 전에만 재시도했으므로 output_may_have_started=False로 정리.
...
# [해설] 헤드리스 shell 정책을 config resolver(`shell.allow_list` 옵션: managed > CLI > env > config.toml 순위, analysis/03)에서 읽는다.
# [해설] 호출: `_make_hitl_decision`(execute 인터럽트 결정 시마다), `run_non_interactive`(승인 정책 도출).
...
# [해설] HITL 인터럽트 1건에 대한 자동 결정(사람 없음). hook이 결정하지 않은 요청에 대해 `_process_hitl_interrupts`가 호출.
# [해설] 규칙: `execute`(shell)만 allow-list로 검증, 목록이 없으면 거부(fail-closed), 그 외 도구는 무조건 승인.
# [해설][주의] 인자에 숨은 유니코드·의심 URL이 있어도 경고만 출력하고 결정 자체는 바꾸지 않는다.
# [해설] 관련 분석: analysis/04-approval-hitl-security.md
def _make_hitl_decision(
    ...
    if action_name == "execute":
        ...
        # [해설] `config.is_shell_command_allowed`: 명령을 allow-list와 대조(파이프·연결 연산자 처리 등 세부는 config.py 참조).
...
# [해설] 도구 인자 전체를 재귀적으로 훑어 위험 유니코드(양방향 제어문자 등)와 URL 키의 도메인 위장(퓨니코드 등)을 경고.
# [해설][SDK] 검사 로직은 `unicode_security.py`(dcode 자체 모듈).
...
# [해설] 서버가 소유한 Hooks v2 인터럽트를 클라이언트 런타임에서 실행해 결과를 hook_response(재개 값)에 넣는다.
# [해설][설계] hook 명령은 클라이언트 프로세스에서 실행된다(서버는 인터럽트로 요청만 함). 관련 분석: analysis/07.
...
# [해설] 대기 중인 HITL 요청마다 ① PermissionRequest hook으로 결정 시도 → ② hook이 중단시키면 ClientHookStopError
# [해설]   → ③ 미해결 항목은 `_make_hitl_decision` 자동 정책으로 채움 → hitl_response[interrupt_id] = {"decisions": [...]}.
...
# [해설] 한 번의 스트림 패스를 끝까지 소비한다(초기 입력 또는 Command(resume=...)).
# [해설] 호출: `_run_agent_loop`. agent는 보통 `client/remote_client.py:RemoteAgent`(RemoteGraph 래퍼, SSE 스트림).
async def _stream_agent(
    ...
    if state.spinner:
        state.spinner.start()
    try:
        # [해설] stream_mode 3종 + subgraphs=True(서브에이전트 네임스페이스 포함). durability="exit": LangGraph가 체크포인트를 스텝마다가 아니라 실행 종료(완료·인터럽트·오류) 시점에 기록.
        # [해설] context는 CLIContext(thread_id·요약 모델·승인 모드 등) — 서버 쪽 make_graph의 runtime context로 전달된다.
        async for chunk in agent.astream(
            ...
            # [해설] compaction 경계 감지: 요약 메시지 구간이 끝났거나 `compact_conversation` 도구 결과가 보이면 SessionStart(COMPACT) hook 실행.
        ...
        if state.summarization_observed:
            await _after_headless_compact(state)
            state.summarization_observed = False
    # [해설] 패스 종료(중단 포함) 시 사용량 원장을 닫는다 → 다음 HITL 재개 패스가 같은 청크를 재생해도 토큰·비용이 이중 집계되지 않음.
    finally:
        _end_reasoning(state)
...
# [해설] 루트 messages 청크가 요약 미들웨어 출력인지(True/False), 판단 대상이 아니면 None.
...
# [해설] 루트의 `compact_conversation` 도구 성공 결과("Conversation compacted."로 시작)면 tool_call_id를 반환(중복 hook 방지 키).
...
# [해설] compaction 후 SessionStart(cause=COMPACT) hook 실행. hook이 중단을 요구하면 ClientHookStopError.
...
# [해설] SessionEnd hook을 최대 SESSION_END_DRAIN_TIMEOUT_SECONDS까지만 기다리고 종료를 진행(한 번만 발화).
...
# [해설] 결과를 받지 못한 tool.use마다 합성 tool.error + tool.result(status=error)를 발화하고 in-flight 맵을 비운다.
# [해설] 호출: 재시도 정리(`_settle_interrupted_tool_hooks`), `_run_agent_loop` finally(스트림 중단·종료).
...
# [해설] 헤드리스 에이전트 루프 본체. 호출: `run_non_interactive`.
# [해설][흐름] 개요: 상태/컨텍스트 준비 → HooksManager 구성 → SessionStart·UserPrompt hook → 초기 스트림
# [해설]   → (인터럽트가 있는 동안) hook 인터럽트 이행 + HITL 자동 결정 → Command(resume)로 재개 반복 → 정리·출력.
async def _run_agent_loop(
    ...
    *,
    ...
    """
    # [해설][흐름] 1) 스트림 상태 생성(quiet면 스피너 없음).
    ...
    # [해설][흐름] 2) 초기 입력: user 메시지 + goal_criteria_request=None(헤드리스는 goal 기능 비사용, 추정) + 선택적 rubric 상태 필드.
    ...
    # [해설][흐름] 3) 모델 미들웨어용 runtime context(CLIContext) 구성.
    ...
    # [해설][흐름] 4) hook 알림/상태 표시 콜백 정의(스피너와 공유).
    ...
    # [해설][흐름] 5) HooksManager 구성: 승인 모드 기본 MANUAL, 세션 식별 고정.
    ...
    state.hooks = hooks or HooksManager.create(
        ...
        # [해설][주의] 보안: 프로젝트 hooks(.deepagents/hooks.json)는 `--trust-project-hooks` 명시 동의가 있을 때만 실행.
        # [해설]   인터랙티브에서 저장된 신뢰는 헤드리스(CI)로 이어지지 않는다.
    ...
    # [해설] hook 관련 그래프 컨텍스트 주입 + approval_mode/auto_approve를 context에 기록(YOLO일 때만 auto_approve True).
    ...
    # [해설][흐름] 6) SessionStart(STARTUP) hook. 추가 컨텍스트가 있으면 system 메시지로 입력 맨 앞에 삽입.
    ...
    # [해설][흐름] 7) UserPromptSubmit hook: 중단 / 컨텍스트 추가(사용자 메시지 바로 앞) / 원래 프롬프트 제거 가능.
    # [해설]   Hooks v2 핸들러가 없으면 레거시 `session.start`/`user.prompt` 이벤트를 보낸다.
    try:
    ...
    try:
        ...
        turns = 1
        # [해설][흐름] 9) HITL 재개 루프: 한도 검사 → 응답 초기화 → hook 인터럽트 이행 → HITL 결정 → Command(resume)로 재스트림.
        # [해설]   resume 페이로드는 {interrupt_id: 값} 매핑이며 hook 응답과 HITL 결정을 합친다(같은 id면 HITL 쪽이 우선).
        ...
        await _end_headless_session(state, SessionEndCause.OTHER)
        raise
    # [해설][흐름] 10) finally: 고아 tool.use 정리, 파싱 안 된 버퍼 로그 등(모든 정리는 원래 예외를 가리지 않도록 가드).
    finally:
        ...
        # [해설] 시도 범위 마무리: 정상 완료면 열린 시도를 커밋(완료 이벤트 유실 보정), 중단이면 미확정 transcript 폐기.
    ...
    # [해설][흐름] 11) 결과 출력: --no-stream 버퍼 flush → (비 quiet) LangSmith 링크·완료 표시·사용량 표.
    ...
    # [해설][흐름] 12) 종료 알림: Notification(AGENT_COMPLETED) hook → 레거시 task.complete → SessionEnd hook → 레거시 session.end.
    # [해설]   알림 hook의 중단 요청은 SessionEnd 발화 후에 다시 raise한다(종료 hook을 건너뛰지 않기 위해).
...
# [해설] 헤더 한 줄(App 버전 | Agent | Model | Thread | Rubric)을 만든다. 기본은 네트워크 조회 없이 스레드 ID만 표시.
...
# [해설] `--startup-cmd` 실행: 에이전트 실행 전 셸 명령을 60초 제한으로 돌리고 출력은 콘솔로 보낸다. 실패·타임아웃은 경고만.
# [해설][주의] 사용자 셸 명령을 승인 없이 실행한다(사용자가 직접 지정한 플래그이므로). 새 세션으로 띄워 종료 시 그룹 단위로 정리.
async def _run_startup_command(
    ...
    *,
    ...
    # [해설] 셸 env에는 dcode 부트스트랩이 가려 둔 사용자 LangSmith env를 복원해 넣는다(`config.restore_user_langsmith_env`).
    try:
    ...
    # [해설] 60초 타임아웃 또는 취소 시 `_terminate_startup_process`로 프로세스 그룹 정리.
    try:
...
# [해설] 헤드리스 실행 진입점. 호출: `main.py:cli_main`(args.non_interactive_message가 있을 때, `--timeout`은 바깥 asyncio.wait_for가 담당).
# [해설] 반환값이 곧 프로세스 exit code: 0 성공/hook 중단, 1 오류, 124 턴 한도, 130 KeyboardInterrupt.
# [해설] 관련 문서: docs_official/code/cli-reference.md(비대화형 옵션), quickstart.md("-n 실행은 매번 새 스레드").
async def run_non_interactive(
    ...
    *,
    ...
    """
    # [해설][흐름] 1) stdio 인코딩 안전화, quiet면 콘솔 전체를 stderr로(답변 텍스트만 stdout).
    ...
    # [해설][흐름] 2) `--startup-cmd` 실행(서버 기동 전).
    if startup_cmd and startup_cmd.strip():
        await _run_startup_command(startup_cmd.strip(), console, quiet=quiet)

    # [해설][흐름] 3) `--skill` 처리: 스킬 탐색(플러그인 스킬 포함, 워커 스레드) → SKILL.md 로드(허용 루트 검사) → 메시지를 envelope로 감싼다.
    # [해설]   관련 분석: analysis/06-memory-skills.md
    message_kwargs: dict[str, Any] | None = None
    ...
    # [해설][흐름] 4) 클라이언트에서도 모델을 생성해 설정 오류(키 누락 등)를 서버 기동 전에 exit 1로 보고하고, 헤더용 모델명을 runtime_state에 반영.
    # [해설][주의] 실제 에이전트가 쓰는 모델 객체는 서버 프로세스(`server_graph._make_graphs_in_environment`)에서 다시 생성된다.
    try:
        result = create_model(
            model_name,
    ...
    # [해설][흐름] 5) 매번 새 스레드 ID(UUID7) — 헤드리스는 resume을 지원하지 않는다. 디버그 로그를 이 스레드 파일에 바인딩.
    ...
    # [해설][흐름] 6) MCP 메타데이터 사전 로드를 서버 기동과 병렬로 시작(표시용 "Loaded N MCP tools"; quiet면 생략).
    ...
    try:
        ...
        # [해설][흐름] 7) shell 정책 → 승인 정책 도출.
        # [해설]   - allow_list 없음 → shell 끔 + 나머지 도구 자동 승인(YOLO)
        # [해설]   - SHELL_ALLOW_ALL("all") → shell 켬 + 자동 승인
        # [해설]   - 제한 목록 → shell 켬 + 서버에서 shell만 인터럽트(interrupt_shell_only), 클라이언트 `_make_hitl_decision`이 대조
        # [해설] 관련 문서: cli-reference.md "-S/--shell-allow-list". 관련 분석: analysis/04-approval-hitl-security.md
        shell_allow_list = _resolve_shell_allow_list()
        ...
        # [해설][흐름] 8) 턴 ID와 hook 세션 식별 생성 → HooksManager(프로젝트 hooks는 명시 동의 시에만).
        ...
        # [해설][설계] PermissionRequest hook이 있으면 두 단축 경로(auto_approve, interrupt_shell_only)를 모두 끈다
        # [해설]   → 모든 게이트 대상 도구 호출이 HITL 인터럽트로 클라이언트에 도달해 hook이 판단할 수 있다.
        ...
        # [해설] 서버로 넘길 구체 allow-list는 interrupt_shell_only 모드일 때만(서버 `ShellAllowListMiddleware`용).
        ...
        # [해설][흐름] 9) RunnableConfig: thread_id + metadata(agent_name, cwd, updated_at 등; `config.build_stream_config`) — sessions.db의 스레드 목록 조회에 쓰인다.
        ...
        # [해설][흐름] 10) 서버 기동(`server_manager.server_session`, interactive=False, ask_user 비활성, cwd=현재 디렉터리).
        # [해설][주의] 헤드리스는 summarization_model·auto_classifier_model을 서버로 넘기지 않는다(TUI 경로 `main.py`는 server_kwargs에 summarization_model 포함). 요약 모델은 `_run_agent_loop`의
        # [해설]   CLIContext(runtime context)로만 전달된다(서버 ServerConfig.summarization_model은 None으로 남음).
        async with server_session(
            ...
            # [해설][흐름] 11) MCP 사전 로드 결과 표시(실패해도 실행은 계속).
            ...
            # [해설][흐름] 12) 파일 조작 추적기(backend=None: 서버 백엔드에 직접 접근하지 않고 도구 메시지로만 추적, 추정) → 에이전트 루프 실행.
    ...
    # [해설][흐름] 13) 예외 → exit code 매핑. `--max-turns` 초과(HITLIterationLimitError)는 GNU timeout과 같은 124.
        ...
        return 0
    # [해설][흐름] 14) 백그라운드 fire-and-forget hook 태스크(tool.use/tool.result 등)를 이벤트 루프 종료 전에 모두 기다린다.
    finally:
