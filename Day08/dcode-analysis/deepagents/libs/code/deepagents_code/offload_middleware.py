...
# [해설] 이 모듈의 역할: dcode의 컨텍스트 컴팩션(요약) 계층. SDK 요약 엔진을 감싸 세 경로를 한 인스턴스로 통합한다.
# [해설] (1) 자동 요약(임계치 초과/프로바이더 오버플로) (2) 모델이 부르는 `compact_conversation` 도구 (3) 서버 HTTP `/offload`
# [해설] 실행 위치: 서버 프로세스(에이전트 그래프 미들웨어 + offload_api HTTP 핸들러). 훅(PreCompact/PreToolUse)은 인터럽트로 클라이언트에서 실행.
# [해설] 주요 진입점 심볼:
# [해설] - `_create_cli_compaction_middleware` : `agent.create_cli_agent`가 호출하는 factory
# [해설] - `CLICompactionMiddleware` : `.name` 별칭으로 SDK "SummarizationMiddleware" 슬롯을 교체하는 미들웨어
# [해설] - `OffloadOperation` / `attach_offload_operation` / `offload_operation_from` : `/offload` 실행기 게시·조회(`offload_api._execute_offload`가 사용)
# [해설] - `_install_summary_*` : LangChain 요약 helper의 private 슬롯(`_summary_model`, `trim_tokens_to_summarize`, 토큰 카운터)을 교체
# [해설][SDK] 기반 심볼: `deepagents.middleware.summarization`의 `SummarizationToolMiddleware`, `create_summarization_middleware`,
# [해설] `create_summarization_tool_middleware`, 그리고 LangChain `langchain.agents.middleware.summarization`.
# [해설] 관련 분석 문서: analysis/02-agent-assembly-sdk-core.md (설계 포인트 1~4, D절 /offload), analysis/07-mcp-hooks-extensions-plugins.md (훅)
# [해설] 관련 공식 문서: docs_official/sdk/context-engineering.md (요약·오버플로), docs_official/code/configuration.md (/offload, conversation_history), docs_official/code/config-file.md (summarization 모델)
...
# [해설] 전용 요약 모델의 컨텍스트 창을 알 수 없을 때 요약 입력 히스토리 상한(토큰). `_summary_trim_limit`에서 사용.
_SUMMARY_TRIM_FALLBACK = 4_000
"""Conservative history budget when a summary model exposes no context limit."""


# [해설][설계] 요약 모델 호출을 dcode 재시도 정책(`model_retry.retry_model_call`)으로 감싸는 Runnable 어댑터.
# [해설] LangChain 요약 helper는 요약 모델에 무조건 `with_retry`(3회)를 거는데, 이를 대체해 `--max-retries`를 존중한다.
# [해설] 설치 위치: `_install_summary_model_retries`가 helper `_summary_model` 슬롯에 넣는다.
class _RetryingModelInvoker(Runnable[LanguageModelInput, AIMessage]):
    ...
    # [해설] 비동기 요약 호출 + dcode 재시도(서버는 주로 이 경로)
    async def ainvoke(
        self,
        input: LanguageModelInput,  # noqa: A002  # `Runnable` keyword contract
        config: RunnableConfig | None = None,
...
# [해설][주의] LangChain/SDK private 속성 교체의 안전장치. 슬롯 이름이 바뀌면 새 속성만 생기고 아무도 안 읽는 "조용한 no-op"이 되므로
# [해설] 교체 전에 존재를 확인해 AttributeError로 크게 실패한다. 이 모듈의 모든 `_install_*` 함수가 사용.
...
# [해설] 요약 helper의 `_summary_model`을 `_RetryingModelInvoker`로 교체. model 인자가 있으면 그 모델(전용 요약 모델)로 요약을 생성한다.
# [해설] 호출자: `_create_cli_compaction_middleware`, `_summarization_for_runtime`, `_LazySummaryModel`(성공/실패 폴백).
...
# [해설][SDK] SDK factory는 `trim_tokens_to_summarize=None`(트리밍 끔)으로 LangChain 기본 4000을 덮는다.
# [해설] 전용 요약 모델은 메인보다 작을 수 있으므로 그때만 그 모델 창에 맞는 상한을 설치한다.
...
# [해설] 요약 모델 `profile["max_input_tokens"]`의 80%(4/5). 나머지 20%는 요약 prompt·직렬화 오버헤드·근사 토큰 카운터 오차 여유분.
...
# [해설] 전용 요약 모델의 provider별 근사 토큰 카운터로 교체(메인 모델 튜닝으로 요약 모델 창을 재면 어긋나므로).
...
# [해설] 테스트용 blockbuster의 `BlockingError`(이벤트 루프에서 블로킹 I/O 감지)인지 클래스 이름으로 판별. 이것은 폴백으로 흡수하지 않고 재발생시킨다.
...
# [해설][설계] 전용 요약 모델(`--summarization-model` 등)을 "처음 요약이 필요할 때" 만든다(시작 비용·자격증명 오류 회피).
# [해설] 생성 또는 호출이 실패하면 메인 모델로 요약하도록 강등하고 경고는 한 번만 남긴다 → 요약 모델 오류가 세션/`/compact`를 망가뜨리지 않음.
# [해설] 설치: `_install_lazy_summary_model`이 helper의 `_create_summary`/`_acreate_summary`를 이 객체의 메서드로 바꿔 끼운다.
class _LazySummaryModel:
    ...
    def __init__(
        ...
        self._warned = False
        # [해설] 오버라이드 설치 전의 메인 모델 튜닝(카운터·트림)과 원래 요약 함수를 캡처해 두어 강등 시 원상 복구에 쓴다.
        helper = summarization._lc_helper
    ...
    # [해설][흐름] 1) 이미 설정됐으면 종료 2) workspace env로 `create_model` 3) 실패 시 메인 모델 재시도 설치 + 1회 경고 4) 성공 시 모델·카운터·트림 설치
    ...
    # [해설] 요약 생성 실패 시 메인 모델 튜닝 복원 + `_configured=False` → 다음 컴팩션에서 오버라이드를 다시 시도한다.
    ...
    # [해설] 동기 요약: 설정 → 호출 → (오버라이드 실패 시) 강등 후 메인 모델로 1회 재호출. 메인 모델 실패는 그대로 raise.
    ...
    # [해설][주의] 스레드 Lock을 이벤트 루프에서 기다리면 루프가 멈추므로 `to_thread`로 획득. 취소되어도 획득 완료 후 해제해 락 누수를 막는다(shield).
    @asynccontextmanager
    ...
    # [해설] 비동기 요약. `_configure`(모델 생성 = 동기 I/O 가능)와 강등을 스레드로 보내 서버 이벤트 루프를 막지 않는다.
...
# [해설] 요약 helper의 요약 생성 hook 2개를 `_LazySummaryModel`의 바운드 메서드로 교체(이 summarizer 인스턴스에만 적용).
...
# [해설] `/offload` 서버 연산이 읽고 쓰는 checkpoint 채널 타입(`messages`, `_summarization_event`, 비용 채널 등을 상속으로 모음).
...
# [해설] `/offload` 결과 상태: compacted(성공) / empty(메시지 없음) / noop(요약할 게 없음) / denied(훅이 거부) / failed(오류).
...
# [해설] 클라이언트(`RemoteAgent.aoffload` → TUI)에 돌려주는 `/offload` 결과 wire 형식.
...
# [해설][설계] 서버 연산이 checkpoint에 쓸 수 있는 채널을 타입으로 제한한다. 핵심 불변식: 이 경로는 `messages`를 절대 쓰지 않는다
# [해설] (요약은 `_summarization_event`로만 표현하고 원본 메시지는 checkpoint에 보존). `offload_api`에 런타임 검사도 백업으로 있다.
...
# [해설] `OffloadOperation.execute` 반환값: state update + 결과 + (아직 쓰지 않은) 아카이브 append 계획.
...
# [해설] 세션 id별 asyncio Lock 레지스트리. 약한 참조라 쓰는 곳이 없으면 자동 정리된다. 프로세스 로컬(서버 워커 간 공유 안 됨).
_archive_locks: WeakValueDictionary[str, asyncio.Lock] = WeakValueDictionary()


# [해설][설계] 같은 세션의 `conversation_history/{session}.md` 읽기→append→쓰기 사이클을 직렬화한다.
# [해설] 자동 요약(`_awrap_with_archive_lock`), compact 도구(`_arun_compact`), `/offload`(`arun_forced_compaction_update`)가 모두 이 락을 공유.
...
# [해설][설계] "요약은 이미 만들었지만 아카이브 파일에는 아직 안 쓴" 상태. `/offload`는 checkpoint 예약을 먼저 한 뒤
# [해설] 이 append를 수행하고, 커밋 실패 시 `_ArchiveAppend.rollback`으로 되돌린다(offload_api의 deferred commit, 추정).
class _PendingArchive(NamedTuple):
    ...
    # [해설] 확정된 아카이브 경로(실패 시 None)로 `_summarization_event` update 생성.
    ...
    # [해설] append 전 스냅샷 읽기(롤백용). 파일 없음은 정상, 그 외 읽기 오류는 예외.
    ...
    # [해설][흐름] 1) 이전 내용 스냅샷 2) `_ArchiveReadGuard`로 감싼 백엔드에 SDK `_aoffload_to_backend`로 append 3) SDK가 None(실패)을 주면 롤백 후 None
...
# [해설] 완료된 append의 되돌리기 정보. 원래 파일이 있었으면 이전 내용으로 덮고, 없었으면 삭제한다.
...
# [해설] `_aplan_forced_compaction_update`의 결과: 요약 텍스트 + 절대 cutoff + 보류된 아카이브.
...
# [해설] 상태를 바꾸지 않은 결과 생성기. HTTP 경계(`offload_api`)가 에이전트 런타임을 만들지 않고도 empty/noop 등을 보고할 수 있게 모듈 수준에 둔다.
...
# [해설] `/offload` 프로토콜 한 라운드의 응답 타입들. interrupt 응답이면 클라이언트가 훅을 실행하고 같은 operation_id로 재요청한다.
...
# [해설] CompositeBackend 객체에 `OffloadOperation`을 매다는 동적 속성 이름.
_OFFLOAD_OPERATION_ATTR = "_dcode_offload_operation"


# [해설][설계] 에이전트 그래프와 HTTP `/offload`가 "같은" 컴팩션·훅 인스턴스를 쓰도록, server_graph와 공유되는 backend 객체에 실행기를 게시한다.
# [해설] 호출자: `agent.create_cli_agent`. 컴팩션이 다른 backend에 묶여 있으면 아카이브 위치가 어긋나므로 ValueError.
...
# [해설] 게시된 실행기 조회. 호출자: `server_graph`(필수 검사), `offload_api`.
...
# [해설] `_summarization_event["cutoff_index"]`(원본 messages 기준 절대 인덱스)를 안전하게 읽는다. bool 오염 방지.
...
# [해설] PreCompact 훅이 오버플로 복구 요약을 거부했을 때, SDK의 오버플로 폴백 핸들러를 통과해 원래 `ContextOverflowError`를 전달하기 위한 운반용 예외.
...
# [해설] run context에서 읽은 현재 모델 설정(`/model`, `/summarization-model` 선택 반영). `_summarization_for_runtime`의 입력.
...
# [해설] `ToolRuntime`(도구 경로)과 LangGraph `Runtime`(서버 연산 경로) 모두를 받기 위한 최소 프로토콜(`.context`만 필요).
...
# [해설] 훅 실행 cwd: run context의 workspace cwd 우선, 없으면 프로세스 cwd.
...
# [해설] `CLIContextSchema.from_payload`로 in-process 객체/원격 dict 모두에서 모델 설정을 추출. context가 없으면 전부 None(시작 설정 사용).
...
# [해설][주의] SDK 아카이브 helper는 "읽기 실패"를 "파일 없음"처럼 다뤄 전체 덮어쓰기(write)를 한다 → 기존 히스토리 유실 위험.
# [해설] 이 어댑터는 읽기 오류(FILE_NOT_FOUND 제외)를 기록하고 이후 write/edit를 RuntimeError로 막아 fail-closed로 만든다.
class _ArchiveReadGuard:
    ...
    # [해설] FILE_NOT_FOUND 외 오류가 있으면 읽기 실패로 기록.
    ...
    # [해설] 쓰기 전 검사: 앞선 읽기가 실패했다면 덮어쓰기 거부.
    ...
    # [해설] 이하 download/write/edit (sync/async) 위임 메서드: 읽기는 실패 기록, 쓰기는 읽기 성공 확인 후에만 위임.
...
# [해설] dcode 컴팩션 미들웨어. SDK `SummarizationToolMiddleware`(compact_conversation 도구 제공)를 상속하면서
# [해설] `.name`을 내부 요약 엔진 이름("SummarizationMiddleware")으로 위장해 SDK 자동 요약 슬롯까지 차지한다.
# [해설][SDK] SDK 문서는 도구 미들웨어를 자동 요약과 "함께" 두라고 하지만, dcode는 둘을 한 인스턴스로 합쳐
# [해설] 자동 요약·compact 도구·/offload가 같은 summarizer 선택(`_summarization_for_runtime`)과 같은 세션 락을 공유한다.
# [해설] 추가 기능: PreCompact 훅(AUTO 트리거), 런타임 모델 추종, 요약 모델 재시도/트림/카운터 주입, 서버 forced compaction 계획.
class CLICompactionMiddleware(SummarizationToolMiddleware):
    ...
    # [해설] `summarization`은 시작 모델 기준 SDK 요약 엔진. `_runtime_summarization`은 런타임 모델용 1-slot memo(키, summarizer).
    def __init__(
        self,
        summarization: SummarizationMiddleware,
        *,
    ...
    # [해설][SDK] 이름 별칭 트릭의 핵심. SDK `graph.py` `_apply_custom_middleware`가 이 이름으로 기본 요약 미들웨어를 제자리 교체한다.
    # [해설] 결과적으로 dcode `agent_middleware` 리스트 끝에 append되어도 스택에서는 Filesystem·SubAgent 다음 3번째 위치에 놓인다.
    @property
    def name(self) -> str:
        """Replace the SDK auto-summarizer while retaining the compact tool."""
        return self._summarization.name

    # [해설] 같은 모델 입력 스냅샷(메시지 수 + 마지막 메시지 id/해시)에 대한 안정 식별자. 훅 재개 시 같은 논리 이벤트로 인식시키기 위한 `logical_event_id`.
    @staticmethod
    ...
    # [해설][흐름] 자동 요약 전에 PreCompact 훅(trigger=AUTO) 실행. 세션 게이트에서 이벤트가 꺼져 있으면 즉시 허용.
    # [해설] `_invoke_hook`은 훅이 필요하면 인터럽트로 클라이언트에 실행을 요청한다(analysis/07). 반환 False = 요약 건너뜀.
    ...
    # [해설][흐름] 임계치 기반 요약이 "실제로 돌지" 미리 판정: 유효 메시지 재구성 → 토큰 계산 → 오래된 tool 인자 절단(절단되면 재계산)
    # [해설] → `_should_summarize` 그리고 cutoff > 0. 돌 경우에만 절단된 messages로 바꾼 request를 반환(훅은 요약이 확정될 때만 부르기 위함).
    # [해설][주의] 여기서 세는 `request.system_message`/`tools`에는 안쪽 미들웨어(Memory/LocalContext 등)가 나중에 붙이는 prompt가 빠져 있을 수 있다(추정, analysis/02 더 볼 거리).
    @staticmethod
    ...
    # [해설] 프로바이더 `ContextOverflowError` 복구 요약 전에도 PreCompact 훅으로 게이트. 거부되면 `_AutoCompactionBlockedError`로 SDK 폴백을 탈출.
    ...
    # [해설][흐름] 동기 모델 호출 래핑. `call_model` = 부모(SDK 도구 미들웨어)의 wrap → compact 도구 prompt/자격 처리 후 안쪽 handler 호출.
    # [해설] 1) 런타임 summarizer 선택 2) 임계치 요약 예정이면 훅 확인 → 거부 시 요약 없이 호출, 허용 시 SDK summarizer가 요약 후 호출
    # [해설] 3) 임계치 미만이면 SDK summarizer에 `gated_handler`를 넘겨, 오버플로가 나면 최초 1회만 훅 게이트 후 SDK 복구 경로로 넘긴다
    def wrap_model_call(  # ty: ignore[invalid-method-override]  # delegates auto summarizer
        ...
        # [해설][흐름] 3) 오버플로 경로: SDK summarizer가 예외를 잡아 요약·재시도하기 전에 훅을 1회만 확인
        ...
        # [해설] 훅이 거부했으면 운반 예외를 풀어 원래 오버플로 예외를 호출자(모델 노드)에 전달.
        try:
    ...
    # [해설] 비동기 자동 요약을 세션 아카이브 락 안에서 실행(compact 도구·/offload와 동시 append 방지).
    @staticmethod
    ...
    # [해설][흐름] 비동기 버전(서버 기본 경로). 동기와 같은 단계이며 차이점: summarizer 선택(`create_model` 가능)을 스레드로 보내고, 요약 경로를 `_archive_lock`으로 직렬화.
    # [해설][주의] 임계치 미만(오버플로 대비) 경로도 락 안에서 모델 호출 전체를 감싸므로, 같은 세션의 compact와 모델 호출이 직렬화된다.
    ...
    # [해설] 모델이 호출한 `compact_conversation`(동기): 런타임 summarizer를 얹은 임시 host로 SDK 구현 실행. asyncio 락은 쓸 수 없어 락 없음.
    ...
    # [해설] `compact_conversation`(비동기): 런타임 summarizer + 세션 아카이브 락.
    ...
    # [해설][설계] SDK `_run_compact`는 `self._summarization`을 읽으므로, 요청별 summarizer를 쓰려면 버리는 host 인스턴스를 만드는 수밖에 없다.
    @staticmethod
    def _compact_host(
        summarization: SummarizationMiddleware,
    ) -> SummarizationToolMiddleware:
    ...
    # [해설] SDK 도구 생성 hook 오버라이드: 같은 이름 `compact_conversation`이지만 위의 런타임 summarizer·락 경로로 실행된다.
    # [해설] 이 도구 인스턴스는 `agent.create_cli_agent`에서 Auto 모드 분류기에 `trusted_compaction_tool`로도 전달된다.
    ...
    # [해설] 요청 단위 summarizer 선택. 컴팩션은 스택 바깥쪽(ConfigurableModelMiddleware보다 바깥)이라 `request.model`에 `/model` 선택이
    # [해설] 아직 반영되지 않았으므로, run context의 모델 설정으로 summarizer를 직접 다시 만든다.
    # [해설] 규칙: 임계치·토큰 계산 = 메인(런타임) 모델, 요약 생성 = 요약 모델 오버라이드(있으면).
    def _summarization_for_runtime(
        ...
        Returns:
            The startup summarizer when neither a runtime model nor a
                ...
                different models.
        """
        # [해설][흐름] 1) 요약 모델 spec 결정: 런타임 INHERIT sentinel → 메인 모델 사용(None) / 런타임 미지정 → 시작 시 spec / 그 외 런타임 값
        # [해설] 둘 다 없으면 시작 summarizer 그대로(가장 흔한 경로)
        ...
        # [해설][흐름] 2) 1-slot memo: 설정이 같으면 이전 summarizer 재사용(매 모델 호출마다 불리므로 `create_model` 비용 회피)
        ...
        # [해설][흐름] 3) 메인 모델: 런타임 spec이 있으면 새로 생성(workspace env 적용), 없으면 시작 summarizer의 모델 공유
        from deepagents_code.config import create_model
        ...
        # [해설][흐름] 4) 런타임 컨텍스트 한도 오버라이드를 새로 만든 모델 profile에만 적용(공유 모델 오염 방지). 요약 트리거 비율이 이 값에 의존.
        ...
        # [해설][흐름] 5) 같은 CompositeBackend로 SDK summarizer 생성(가드 래퍼를 넘기면 artifacts_root 판별이 깨짐 — 위 원문 주석) 후 요약 모델 설치
    ...
    # [해설] `/offload`용 "계획" 단계: 요약은 생성하되 아카이브는 쓰지 않는다. 실패는 ToolMessage가 아니라 예외로 올린다(도구 노드가 없으므로).
    # [해설] 호출자: `OffloadOperation.execute`, `arun_forced_compaction_update`.
    async def _aplan_forced_compaction_update(
        ...
        if not messages:
            ...
            raise ValueError(msg)
        # [해설][흐름] 1) 이전 요약 이벤트를 적용한 유효 메시지 2) 보존 구간을 뺀 cutoff 계산(0이면 요약할 것 없음)
        ...
        # [해설][흐름] 3) 상대 cutoff → 원본 messages 기준 절대 cutoff 변환. 이전 이벤트보다 전진하지 않으면 요약-의-요약만 되므로 중단
        ...
        if state_cutoff <= _event_cutoff(event):
            ...
            return None
        # [해설][흐름] 4) 요약 대상 분리 → 요약 모델 호출(비용 발생) → 아카이브 append는 보류 객체로 반환
    ...
    # [해설] 계획 + 즉시 아카이브 쓰기를 한 번에 하는 과거 호환 경로(직접 호출자용). HTTP 경로는 계획만 쓰고 checkpoint 예약 후 쓰기를 한다.
    # [해설][주의] 아카이브 쓰기가 실패해도 요약 update는 반환한다 → 요약된 원본 메시지를 파일에서 복구할 수 없게 되며 에러 로그만 남는다.
    ...
    # [해설][SDK] `/offload` state update 생성: SDK `_build_new_messages_with_path`로 요약 HumanMessage(아카이브 경로 안내 포함)를 만들고
    # [해설] `_summarization_event{cutoff_index(절대), summary_message, file_path}` + `_summarization_session_id`만 반환한다(messages는 건드리지 않음).
    @staticmethod
...
# [해설] factory. 호출자: `agent.create_cli_agent`(메인 에이전트 1회).
# [해설] 1) SDK `create_summarization_tool_middleware(model, backend)`로 모델 인지 기본값(0.85/0.10 트리거 등)을 가진 엔진+도구 생성
# [해설] 2) 엔진의 요약 모델 호출에 dcode 재시도 설치 3) 엔진·SDK 시스템 prompt를 그대로 넘겨 `CLICompactionMiddleware`로 재포장
...
# [해설] `/offload` 훅 호출용 가짜 tool-call id를 결정적으로 만들기 위한 UUID 네임스페이스.
...
# [해설][설계] 훅 재개 호환 id: 같은 시도의 재개 라운드에서는 동일(재실행이 처음부터 다시 돌기 때문), 다른 시도끼리는 달라야 한다.
# [해설] `offload_api._execute_offload`가 설정하는 `checkpoint_ns = dcode_offload:{operation_id}`에서 uuid5로 파생한다.
# [해설][주의] 네임스페이스가 없으면 랜덤 id로 폴백 → 훅이 설정된 사용자만 재개 불일치로 실패하므로 경고를 크게 남긴다.
...
# [해설] 서버 HTTP `/offload`의 실행기. 에이전트와 같은 `CLICompactionMiddleware`·`ServerHooksMiddleware` 인스턴스를 보관한다.
# [해설] 생성·게시: `agent.create_cli_agent` → `attach_offload_operation`. 호출: `offload_api._execute_offload` → `execute`.
class OffloadOperation:
    ...
    # [해설][설계] 실제 모델 턴이 없는 `/offload`에서도 훅 정책을 적용하려고, 가짜 AIMessage(`compact_conversation(force=True)` 호출)를
    # [해설] `ServerHooksMiddleware.aafter_model`에 넣어 PreCompact/PreToolUse 결정을 얻는다. 훅이 클라이언트 실행을 요구하면
    # [해설] `HookTransportInterruptError`가 올라가고, HTTP 응답이 interrupt가 된다.
    async def _run_hooks(
        ...
        # [해설][주의] 결정 채널이 없으면 허용으로 간주하지 않고 거부(fail-closed). deny면 denied, additionalContext는 실을 곳이 없어 버림.
    ...
    # [해설][흐름] `/offload` 1회 실행:
    # [해설] 1) 유효 메시지·토큰(근사) 계산, 메시지 없으면 empty
    # [해설] 2) 훅 게이트(거부/실패면 상태 불변 결과)
    # [해설] 3) forced compaction 계획(요약 생성) — 실패 시 failed
    # [해설] 4) 계획 없음 → noop
    # [해설] 5) 새 이벤트로 결과 통계 계산 후 허용 채널 2개만 update로 반환 + 아카이브 append는 호출자(offload_api)가 수행
    async def execute(
        self,
        state: _OffloadState,
        ...
        # [해설][주의] 여기서 file_path는 아직 None(아카이브 미기록). 실제 경로는 offload_api가 아카이브를 쓴 뒤 `archive.update(append.path)`로 확정한다(`offload_api`의 아카이브 커밋 단계).
        # [해설] 따라서 `archive_path` 결과 필드도 이 시점엔 None이 될 수 있다.
        ...
        # [해설] `_session_cost_usd`는 `OffloadStateUpdate`에 허용돼 있지만 여기서는 쓰지 않는다(요약 비용 기록은 다른 경로, 추정).
