...
# [해설] 이 모듈의 역할: dcode 설정 서브시스템의 "허브". .env 부트스트랩(신뢰 경계·거부 키), LangSmith 트레이싱 env 정리,
# [해설] 글리프/배너 같은 UI 상수, 스트림 config(메타데이터) 조립, 셸 allow-list 파싱, 재시도 예산 해석,
# [해설] 자격증명 스냅샷(`Credentials`)과 `/reload`, 기본 모델 자동 선택, 그리고 모델 생성 단일 진입점 `create_model`을 모두 담는다.
# [해설] 실행 프로세스: 클라이언트(CLI/TUI/headless)와 LangGraph 서버 프로세스 양쪽에서 import된다.
# [해설] 서버 쪽은 `use_environment`로 워크스페이스 env 스냅샷을 바인딩해 `os.environ`을 건드리지 않고 읽는다.
# [해설] 주요 진입점: `create_model`, `ModelResult`, `credentials`/`runtime_state`/`console` lazy 전역(`_LazyProxy`, 모듈 하단),
# [해설] `_ensure_bootstrap`, `Credentials.reload_from_environment`, `build_stream_config`, `detect_provider`,
# [해설] `is_shell_command_allowed`, `get_tracing_status`.
# [해설] 호출자: `main.py`(CLI 부팅), `app.py`(TUI·/reload·/model), `agent.py`(그래프 조립), `configurable_model.py`(런타임 모델 교체),
# [해설] `server.py`/`_server_config.py`(서버 env 전달) 등 거의 모든 모듈 (추정: 전수 grep은 하지 않음).
# [해설] 관련 분석 문서: `analysis/03-config-models-credentials.md`(핵심), `analysis/01-boot-client-server.md`(부팅), `analysis/04-approval-hitl-security.md`(셸 allow-list).
# [해설] 관련 공식 문서: `docs_official/code/configuration.md`, `config-file.md`, `credentials.md`, `providers.md`.
# [해설][주의] 7천 줄이 넘는 단일 모듈이라 import 비용이 크다. 그래서 무거운 의존성(langchain, dotenv, model_config 등)은 대부분 함수 안에서 지연 import한다.
...
# [해설][설계] 모듈 최상단 import는 가벼운 내부 상수 모듈로만 제한한다(`_constants`, `_env_vars`, `_paths`, `_git`, `_version`).
...
# [해설] 타입 힌트 전용 import. 런타임 순환 import(config_manifest ↔ config 등)를 피하려는 목적이다.
if TYPE_CHECKING:
...
# [해설][흐름] `_ensure_bootstrap()`가 한 번 실행되며 채우는 프로세스 전역 상태.
# [해설] LangSmith 원본 값(launch/user)을 기억해 두었다가, 서버로 넘기거나(`_encode_user_langsmith_env`) 사용자 명령 실행 시 복원(`restore_user_langsmith_env`)하는 데 쓴다.
@dataclass
...
# [해설] 아래 모듈 전역들은 모두 "부트스트랩/dotenv 로더가 무엇을 주입했는지"를 추적한다. `/reload` 시 주입값만 정확히 걷어내기 위해 필요하다.
_bootstrap_state = _BootstrapState()
"""State captured and mutated by lazy bootstrap."""

# [해설][주의] TUI의 prewarm 워커 스레드가 메인 스레드와 동시에 `credentials`에 접근할 수 있어 락이 필요하다.
...
# [해설] `_load_dotenv`가 `os.environ`에 새로 쓴 key→value. `_strip_dotenv_loaded_values`가 이 값과 "같을 때만" 제거하므로 사용자가 중간에 바꾼 값은 보존된다.
...
# [해설][설계] 서버 프로세스는 보안상 `PYTHONPATH`를 제거하고 뜨지만(`server._SERVER_ENV_DENYLIST`), 사용자의 원래 값은 이 캐리어 env로 운반해 `execute` 서브프로세스에만 재적용한다.
...
# [해설][흐름] 클라이언트 → 서버로 "사용자 명령용 LangSmith 설정"을 JSON으로 전달하는 비공개 env. 인코딩 `_encode_user_langsmith_env`, 디코딩 `_decode_user_langsmith_env`.
...
# [해설] LangChain/LangSmith SDK가 트레이싱 on/off를 판단하는 env 4종. 고아 트레이싱 끄기(`_disable_orphaned_tracing`)와 상태 판단(`_tracing_enabled_from`)에서 사용.
...
# [해설] 에이전트 트레이싱과 "사용자 본인 명령(execute)"의 트레이싱을 분리하기 위해 저장·복원하는 LangSmith selector 전체 목록.
# [해설] `_TRACING_RECONCILED_ENV_VARS`가 이 튜플을 그대로 재사용한다(두 목록이 어긋나지 않게).
...
# [해설] env 매핑에서 LangSmith selector만 스냅샷으로 뽑는다. 없는 키는 None으로 남겨 "적용 시 제거"를 표현한다(`_apply_env_values`와 짝).
...
# [해설][설계] 모든 `.env`(프로젝트·전역)에서 주입 금지인 키. 대부분 "환경변수 → 코드 실행"으로 이어지는 벡터(동적 링커, 인터프리터 경로, 셸 시작 훅, git 헬퍼)이다.
# [해설] 신뢰 루트 `DEEPAGENTS_HOME`, 로더 자신의 토글 `READ_PROJECT_DOTENV`, 서버 캐리어 env 2종도 포함된다. 판정은 `_is_dotenv_denied_env_key`.
# [해설] 관련 공식 문서: `docs_official/code/configuration.md` "Loading order and precedence"의 거부 키 목록(캐리어 `DEEPAGENTS_USER_LANGSMITH_ENV`는 코드에만 있음, analysis/03 참고).
...
# [해설] 번호가 붙는 `GIT_CONFIG_KEY_n` 계열은 집합으로 열거할 수 없어 접두사로 막는다. `str.startswith(tuple)`로 한 번에 검사한다.
...
# [해설] 전역 거부 판정. 대문자로 정규화한 뒤 비교한다 — Windows에서 `os.environ`이 키를 대문자로 바꾸므로 소문자 우회를 막기 위함.
# [해설] 호출자: `_dotenv_environment.apply_dotenv`.
...
# [해설] 거부된 키를 보고한다. 프로젝트 `.env`(비신뢰)는 debug 로그만, 전역 `.env`(사용자 본인 파일)는 stderr 경고까지 출력한다.
# [해설][주의] 값은 공격자가 통제할 수 있으므로 로그에는 키 이름만 남긴다.
...
# [해설] LangGraph 업스트림 recursion 기본값 env. 프로젝트 `.env`에서는 금지(아래 `_PROJECT_DOTENV_DENIED_ENV_KEYS`).
...
# [해설][설계] 프로젝트 `.env`에서만 금지인 키(전역 `.env`·셸에서는 허용). "사용자 수준 보안 결정"을 클론한 리포가 바꾸지 못하게 한다:
# [해설] 프로젝트 MCP 허용/거부 목록, Auto 승인 분류기 모델·timeout, forked subagent 모드, recursion 기본값, TERM_PROGRAM(트레이스 메타데이터).
# [해설] 관련: analysis/03 "dotenv 신뢰 경계", analysis/04(Auto 분류기), analysis/07(MCP 신뢰).
...
# [해설] start_path부터 루트까지 올라가며 가장 가까운 `.env`를 찾는다(프로젝트 `.env` 탐색). 검사 실패한 후보는 경고 후 건너뛴다.
...
# [해설] 전역 `.env` 경로(`~/.deepagents/.env` 또는 `DEEPAGENTS_HOME` 기준 프로필). import 시점에 고정되므로 dotenv가 경로를 옮길 수 없다.
# Frozen before either dotenv layer is inspected.
_GLOBAL_DOTENV_PATH = PATHS.profile.dotenv_file


# [해설] 프로젝트 `.env`와 전역 `.env`가 같은 파일인지(예: 홈 디렉터리에서 실행) 판정. 판단 불가(OSError)면 "같다"로 보고 전역 신뢰를 주지 않는다(fail-closed).
...
# [해설][설계] 워크스페이스 스코프 env 스냅샷을 담는 ContextVar. 서버가 여러 워크스페이스를 동시에 다룰 때 `os.environ`을 오염시키지 않으려고 쓴다.
_active_environment: ContextVar[Mapping[str, str] | None] = ContextVar(
    "deepagents_code_active_environment", default=None
)


# [해설] 현재 바인딩된 스냅샷이 있으면 그것을, 없으면 live `os.environ`을 반환. 자격증명/설정 리더들의 공통 env 소스(`configuration/providers.py` `EnvProvider`도 사용, analysis/03).
...
# [해설] 스코프 바인딩 여부. `create_model` 등은 이 값에 따라 `apply_stored_credentials`(os.environ 쓰기)를 건너뛰고 스냅샷 기반 경로를 탄다.
...
# [해설] 워크스페이스 env 스냅샷을 블록 동안 바인딩하는 컨텍스트 매니저. MappingProxyType으로 얼려 읽기 전용으로 만든다.
# [해설] 호출자: 서버 쪽 그래프 구성, `configurable_model.py`의 지연 모델 교체 등(요청 시점에 재진입). `None`이면 아무 것도 바인딩하지 않는다.
@contextmanager
...
# [해설] Windows는 env 키 대소문자를 구분하지 않으므로 대문자로 정규화, POSIX는 그대로.
...
# [해설] dotenv `${VAR}` 확장용 매핑. 참조 이름을 호스트 규칙대로 정규화해 조회한다(Windows에서 `${proxy_url}` → `PROXY_URL`).
...
# [해설] 한 `.env` 파일을 파싱하고 `${VAR}`를 "명시적으로 넘긴 environ"에 대해서만 확장한다.
# [해설][설계] python-dotenv의 기본 interpolation은 `os.environ`을 섞기 때문에 스코프 스냅샷이 무력화된다 → 직접 구현. 호출자: `_dotenv_environment`.
def _dotenv_values_from(
    ...
    # [해설][흐름] 파일 안의 앞선 키는 뒤 키의 확장에 쓰일 수 있지만, environ에 이미 있는 키는 environ 값이 우선(실제 적용 결과와 동일하게).
...
# [해설][흐름] 프로젝트/전역 dotenv 스택을 주어진 env 매핑 위에 얹어 "새 유효 env"를 반환한다(원본 불변). 실제 `os.environ` 반영은 `_load_dotenv`가 한다.
# [해설] 우선순위: 호출자 env(셸) > 프로젝트 `.env` > 전역 `.env` — "이미 있는 키는 건너뛰기(first-write-wins)"로 구현.
# [해설] 호출자: `_load_dotenv`(부팅·/reload), `_preview_dotenv_environ`(서버 워크스페이스 미리보기).
def _dotenv_environment(
    *,
    ...
    """
    # [해설][흐름] 1) 호출자 env를 복사하고, 확장 기준(baseline)을 "파일 적용 전" 상태로 고정한다.
    ...
    # [해설][흐름] 파일 하나를 적용하는 내부 함수: 파싱 실패 시 unreadable에 기록, 전역 거부 키 → 프로젝트 전용 거부 키 → 이미 존재하는 키 순으로 걸러낸다.
    def apply_dotenv(dotenv_path: Path | None, *, is_project: bool) -> None:
        ...
        for key, value in values.items():
            ...
            if is_project and key.upper() in _PROJECT_DOTENV_DENIED_ENV_KEYS:
                ...
                continue
            # [해설] first-write-wins: 셸 값이나 먼저 적용된 프로젝트 `.env` 값이 있으면 덮어쓰지 않는다.
            if key in env:
                continue
            env[key] = value
    ...
    # [해설][흐름] 2) 프로젝트 `.env` 탐색. 전역 파일과 동일 파일이면(global_is_project) 전역 쪽 적용을 생략해 이중 적용·신뢰 승격을 막는다.
    ...
    # [해설][흐름] 3) 전역 `.env`에서 `READ_PROJECT_DOTENV` 한 키만 프로젝트 파일보다 먼저 읽는다(보안 통제 순서).
    ...
    try:
        ...
        # [해설][주의] 전역 `.env`를 읽지 못하면 "false"로 fail-closed → 프로젝트 `.env`를 건너뛴다. managed/셸 export는 여전히 이 값보다 우선한다.
    ...
    # [해설][흐름] 4) `resolve_read_project_dotenv`(config_manifest.py): managed → env → 전역 .env 값 → config.toml → 기본값(true) 순으로 토글을 결정한다.
    ...
    # [해설][흐름] 5) 전역 `.env` 적용 전의 "프로젝트 층"을 따로 복사(사용자 명령용 LangSmith 캡처에 사용) → 6) 전역 `.env` 적용.
...
# [해설] `os.environ`을 바꾸지 않고 "특정 경로에서라면 유효했을 env"를 계산한다. 트레이싱 publish와 이전 dotenv 주입을 되돌린 복사본에서 시작한다.
# [해설] 호출자: `Credentials.preview_reload_from_environment`, 서버 워크스페이스 env 구성 (추정).
...
# [해설] 스냅샷 적용 유틸: None은 "키 제거", 문자열은 "쓰기". 빈 문자열 `LANGSMITH_API_KEY`와 부재는 SDK에서 의미가 다르기 때문.
...
# [해설] 이 로더가 주입한 값 중 "아직 그대로인 것"만 제거한다. /reload 전이나 자식 프로세스 env를 만들 때 셸 원본 상태로 되돌리는 용도.
...
# [해설] `DEEPAGENTS_CODE_` 접두 env 우선 조회. 접두 키가 "존재"하면 빈 값이어도 채택(→ None 반환, 표준 변수를 가림).
# [해설] `model_config.resolve_env_var`의 매핑 버전(스냅샷용). 공식 문서 `configuration.md` "`DEEPAGENTS_CODE_` prefix"와 일치.
...
# [해설] 부트스트랩이 `LANGSMITH_PROJECT`를 에이전트용 프로젝트(`DEEPAGENTS_CODE_LANGSMITH_PROJECT`)로 덮어쓴 경우, 사용자의 원래 프로젝트명을 돌려준다.
...
# [해설][흐름] dotenv 스택을 실제 `os.environ`에 반영하는 함수. 호출자: `_ensure_bootstrap`(최초), `Credentials.reload_from_environment`(refresh_loaded=True).
# [해설] 부작용: `os.environ`, `_dotenv_loaded_values`, `_dotenv_provenance`, (capture 시) `_bootstrap_state.user_langsmith_env` 갱신.
def _load_dotenv(
    *,
    start_path: Path | None = None,
    ...
    """
    # [해설][흐름] 1) /reload: 이전에 주입한 값을 먼저 걷어내 "파일에서 지운 키"가 남지 않게 한다.
    if refresh_loaded:
        _strip_dotenv_loaded_values(os.environ)
        _dotenv_loaded_values.clear()
        _dotenv_provenance.clear()

    # [해설][흐름] 2) 현재 env를 baseline으로 고정하고, 필요하면 launch 시점 LangSmith 값을 먼저 캡처한다.
    ...
    # [해설][흐름] 3) baseline에 없던 키(= dotenv가 새로 추가한 키)만 os.environ에 쓰고 출처를 기록한다.
...
# [해설] 아래 상수들은 LangSmith 트레이싱 env 정리(접두 → 표준 이름 브리지, 워크스페이스별 publish)에 쓰이는 이름 테이블이다.
...
# [해설] LangSmith SaaS 리전 엔드포인트. `/auth`와 CLI `--base-url`의 `us`/`eu` 별칭 해석(`normalize_langsmith_endpoint`)에 쓰인다.
...
# [해설] `us`/`eu` 별칭 → 정규 URL, 그 외는 공백만 제거해 반환. 호출자: CLI auth 명령과 TUI `/auth` 프롬프트(docstring 기준).
...
# [해설] 값이 SDK 기본 US 엔드포인트인지 판정. 프로필에 기본 URL이 적혀 있어도 "커스텀 엔드포인트"로 오판하지 않기 위함.
...
# [해설][주의] 보안 가드: 저장된 LangSmith API 키가 http(s)가 아닌/호스트에 공백이 있는 값과 짝지어져 엉뚱한 곳으로 전송되지 않게 저장 시점에 거부한다.
...
# [해설][설계] 워크스페이스 전환 시 os.environ에 publish/정리하는 selector 집합. 사용자 명령 복원 집합과 동일 객체를 써서 한쪽만 누락되는 버그를 차단.
...
# [해설] `reconcile_tracing_environment`가 publish한 값을 (아직 그대로라면) 원래 값으로 되돌린 env 복사본을 만든다.
...
# [해설] 스냅샷에서 SDK에 publish할 표준 selector 값을 계산. 접두 override가 허용된 변수만 `_resolve_env_var_from`으로 해석한다.
...
# [해설][흐름] 활성 워크스페이스의 트레이싱 설정을 os.environ에 게시한다. LangSmith SDK는 os.environ만 읽기 때문에, 스냅샷 판단과 SDK 동작이 어긋나지 않게 맞춘다.
# [해설] 부작용: os.environ 쓰기/삭제, `_reconciled_tracing_values` 기록, SDK env 캐시 초기화. 호출자: 서버 워크스페이스 전환 경로 (추정).
...
# [해설] LangSmith SDK의 `functools.cache`된 env 읽기를 비운다. 실패하면 이전 워크스페이스 설정이 남을 수 있어 경고한다.
...
# [해설] LangSmith SDK의 프로필(client config)에서 부트스트랩이 쓰는 필드만 기술한 구조적 타입.
...
# [해설] 터미널/TUI에 새어 나오면 안 되는 SDK 로거 목록. `_quiet_sdk_logging`이 NullHandler(또는 debug 파일 핸들러)를 붙인다.
...
# [해설] MCP 전송 계층 로거 이름. 앱 종료 시 레이스로 발생하는 무의미한 traceback만 선택적으로 거른다(analysis/07 MCP 관련).
...
# [해설] anyio의 closed/broken resource 예외 타입을 지연 import + 캐시. 시작 경로에서 anyio import 비용을 피하려는 목적.
@functools.cache
...
# [해설] MCP 트랜스포트 로거용 필터. "지정 메시지 + anyio 스트림 종료 예외" 조합일 때만 레코드를 버린다(실제 네트워크 오류는 유지).
class _McpShutdownRaceFilter(logging.Filter):
    ...
    # [해설] 예외 그룹이면 모든 leaf가 스트림 종료 예외일 때만 True(실제 오류가 섞이면 보존).
    @classmethod
...
# [해설] MCP 로거에 필터를 설치(중복 방지). debug 모드면 오히려 제거해 debug 로그에 기록되게 한다.
...
# [해설] SDK 로깅 소음 제어. `DEEPAGENTS_CODE_DEBUG`가 켜져 있으면 debug 파일 핸들러로 보내고, 아니면 NullHandler를 붙인다.
# [해설] 호출자: `_ensure_bootstrap`(dotenv 반영 후 재구성) 등.
...
# [해설] LangSmith SDK 비공개 모듈 `langsmith.client._profiles`에서 활성 프로필 설정을 읽는다. SDK가 없거나 구조가 다르면 None.
# [해설][주의] env가 주어지면 `unittest.mock.patch.dict`로 os.environ을 일시 교체한다 — 프로세스 전역 변경이라 스레드 안전하지 않다 (추정).
...
# [해설] LangSmith 프로필에 API key 또는 OAuth 토큰이 있는지. 고아 트레이싱 판정에서 "env 키가 없어도 프로필 인증이 있으면 유지"에 쓰인다.
...
# [해설] 프로필이 SDK 기본값이 아닌 커스텀 엔드포인트를 가리키는지.
...
# [해설] "트레이싱은 켜졌는데 자격증명이 없어 꺼버렸다"는 TUI 알림 문구. `langsmith` CLI 설치 여부로 안내를 바꾼다.
...
# [해설] 일회성 알림을 꺼내고 비운다(TUI가 시작 후 한 번 표시). 호출자: app.py (추정).
...
# [해설] 설정돼 있는 트레이싱 on 플래그를 전부 "false"로 덮어쓴다(삭제가 아니라 false — 이후 로더가 다시 채우지 못하게).
...
# [해설] 캐리어 JSON의 한 매핑이 selector 전체를 str|None으로 갖췄는지 검증. 클라이언트→서버 경계 입력이므로 엄격히 검사한다.
...
# [해설] launch 스냅샷이 아직 유효하게 캡처되지 않았을 때만 주어진 env에서 채운다(이미 있으면 보존 — /reload에서도 최초 launch 값 유지).
...
# [해설][흐름] 서버 측: 캐리어 JSON을 `(launch, user)` 쌍으로 디코딩. 키 구성이 정확히 {"launch","user"}가 아니면 None(버전 불일치 등).
...
# [해설] 캐리어에 담긴 사용자 LangSmith API 키들을 뽑아 Auto 모드 출력 redaction 대상에 추가하게 한다(비밀 누출 방지). 호출자: 승인/redaction 경로 (추정).
...
# [해설][흐름] 클라이언트 측: 서버 subprocess를 띄울 때 캐리어 env 값을 만든다. 부트스트랩 캡처가 없으면 서버 시작 자체를 거부(RuntimeError).
# [해설][설계] 캡처 실패 원인(`_bootstrap_state.error`)을 체인해 증상만이 아닌 원인을 보여준다.
...
# [해설] 사용자 명령 env에서 LangSmith selector(및 `DEEPAGENTS_CODE_` 접두 버전)를 제거.
...
# [해설][주의] 캐리어가 깨졌을 때의 경고. docstring이 스스로 인정하듯 서버 subprocess의 stderr는 `--debug`에서만 보이므로 사실상 사용자에게 안 보이는 알려진 갭이다.
...
# [해설][흐름] 서버의 `execute`(사용자 셸 명령) env를 만들 때 호출(`agent.py`). 에이전트용 LangSmith 키/프로젝트가 사용자 명령에 새지 않도록
# [해설] launch 셸 값 > 프로젝트 `.env` 값으로 selector를 재구성한다(전역 `.env`는 에이전트 설정이므로 제외).
def restore_user_langsmith_env(
    ...
    """
    # [해설][흐름] 1) 캐리어가 있으면(서버 프로세스) 그 값을, 없으면(in-process, 예: --acp) 이 프로세스의 부트스트랩 상태를 사용.
    ...
    if encoded is not None:
        ...
        # [해설][주의] 디코딩 실패 시 fail-closed: 에이전트 키로 폴백하지 않고 LangSmith 인증 없이 실행한다.
        if decoded is None:
    ...
    # [해설][흐름] 2) 접두 selector 제거 → launch 값 적용 → 3) start_path가 있으면 프로젝트 `.env`만(include_global=False) 다시 계산해 하위 층으로 얹는다.
...
# [해설][흐름] "트레이싱 플래그는 켜졌는데 키가 없다"면 플래그를 false로 바꿔 401 폭주를 막는다. 커스텀/레플리카 엔드포인트가 있으면 키 없이도 업로드 가능하므로 건드리지 않는다.
# [해설] 부작용: os.environ, `_orphaned_tracing_disabled_notice`(TUI 일회성 알림). 호출자: `apply_stored_langsmith_auth`.
def _disable_orphaned_tracing() -> None:
    ...
    # [해설] env 키 또는 LangSmith 프로필 인증이 있으면 정상 → 유지.
    has_key = any(
...
# [해설] 트레이싱 중인데 프로젝트가 비어 있으면 `LANGSMITH_PROJECT_DEFAULT`(config_manifest, "deepagents-code" 추정)로 채워 UI 표시와 실제 적재 위치를 일치시킨다.
...
# [해설][흐름] LangSmith 관련 적용 파이프라인(부트스트랩과 `/auth` 저장 경로가 공유):
# [해설] 접두 env 브리지 → 저장된 서비스 키(auth.json)를 env로 → 저장 키 기반 트레이싱 on/엔드포인트/프로젝트 → 고아 트레이싱 끄기 → 기본 프로젝트 → 비밀 redaction 구성.
# [해설][SDK] `model_config.apply_stored_service_credentials` 참고.
...
# [해설] 접두 값과 표준 값이 다를 때 "접두 값이 이긴다"는 경고. `SUPPRESS_ENV_OVERRIDE_WARNING`으로 끌 수 있다.
...
# [해설] `DEEPAGENTS_CODE_LANGSMITH_API_KEY` 등 접두 LangSmith env를 SDK가 읽는 표준 이름으로 복사한다(빈 문자열도 복사 — 명시적 비활성화 의미).
# [해설][주의] os.environ의 표준 변수를 직접 덮어쓴다. 사용자 명령에는 `restore_user_langsmith_env`가 원래 값을 되돌린다.
...
# [해설][흐름] `/auth`로 LangSmith 키를 저장했다면 트레이싱을 기본 on으로 켠다. 명시적 false 플래그가 있으면 존중(키는 유지, 일시 중지).
# [해설][주의] 키의 유효성은 검사하지 않는다(시작 성능). 잘못된 키면 트레이스가 조용히 버려진다.
def _apply_stored_langsmith_tracing(*, replace_project: bool = False) -> None:
    ...
    # [해설] env로만 준 키는 여기서 아무 것도 하지 않는다(기존 동작 유지). 접두/표준 env 키가 저장 키와 다르면(`_stored_langsmith_key_is_suppressed`) 저장 키가 적용되지 않은 것이므로 중단.
    ...
    # [해설] 트레이싱 플래그를 3값(True/False/None)으로 분류: 하나라도 False면 opt-out, True가 없으면 LANGSMITH_TRACING=true를 켠다.
    flags = [
    ...
    # [해설] 프로젝트: `/auth` 즉시 저장(replace_project=True)이면 저장값으로 교체/제거, 시작 시에는 env에 없을 때만 채운다.
...
# [해설] LangSmith 서비스 키에 대해서는 "접두 env > 저장 키 > 표준 env" 규칙이 적용된다: 접두 env가 존재하고 저장 키와 다르면 억제.
# [해설][설계] 모델 프로바이더 키와 달리 서비스 키에는 접두 env 우선 가드가 있다(analysis/03 문서 대조표 "Provider 키 해석 순서" 참고).
...
# [해설] 저장된 LangSmith 엔드포인트(EU 리전 등)를 `LANGSMITH_ENDPOINT`에 적용. 시작 시에는 env 값이 우선, `/auth` 저장 시에는 교체.
# [해설] 보조 이름 `LANGCHAIN_ENDPOINT`는 항상 비워 SDK가 오래된 값을 읽지 않게 한다.
...
# [해설][흐름] 1회성 부트스트랩(스레드 안전, idempotent). `_get_credentials()`가 `credentials` 첫 접근 시 호출한다(모듈 하단 lazy 전역).
# [해설] 순서: launch LangSmith 캡처 → 서버 프로젝트 컨텍스트로 start_path 결정 → `_load_dotenv` → debug 로깅 재구성 → SDK 로깅 소음 제거
# [해설] → 원래 LANGSMITH_PROJECT 기억 → `DEEPAGENTS_CODE_LANGSMITH_PROJECT`로 덮어쓰기 → `apply_stored_langsmith_auth`.
# [해설][설계] 실패해도 앱은 뜬다(예외는 `_bootstrap_state.error`에 보관, done=True는 finally에서).
# [해설] 관련: analysis/03 "A. 부팅", analysis/01.
def _ensure_bootstrap() -> None:
    ...
    # [해설] double-checked locking: 락 밖 빠른 경로 + 락 안 재확인.
    with _bootstrap_lock:
        ...
        try:
            ...
            # [해설][흐름] 2) 서버 프로세스라면 클라이언트가 넘긴 사용자 cwd(`get_server_project_context`)를 프로젝트 `.env` 탐색 시작점으로 쓴다. 없으면 None → cwd.
...
# [해설] 여기부터는 설정과 별개인 "UI/표시 상수" 구역(모드 접두사, 글리프, 배너)과 트레이스 메타데이터 빌더가 이어진다.
if TYPE_CHECKING:
...
# [해설] TUI 입력창의 모드 트리거 문자: `!!`=기록 안 남기는 셸(incognito), `!`=셸, `/`=슬래시 명령. 사용처: 입력 위젯의 모드 감지(analysis/09).
...
# [해설] import 시점 불변식 검사: 두 테이블의 키가 다르면 즉시 ValueError(개발자 실수 조기 발견).
...
# [해설] 긴 접두사 우선 매칭을 위해 import 시 한 번 정렬(`!!`가 `!`보다 먼저).
...
# [해설] 입력 텍스트의 모드 접두사를 찾는다. 키 입력마다 호출되므로 미리 정렬한 튜플을 순회만 한다.
...
# [해설] TUI 문자셋 모드. `DEEPAGENTS_CODE_UI_CHARSET_MODE`(또는 `UI_CHARSET_MODE`)로 강제하거나 auto 감지(`_compute_charset_mode`).
...
# [해설] TUI 전역에서 쓰는 글리프 묶음. 위젯은 문자 리터럴 대신 `get_glyphs().checkmark`처럼 접근해 ASCII 터미널 대응을 자동화한다.
...
# [해설] 7-bit ASCII 전용 터미널용 대체 글리프 세트(각 필드는 가능한 한 폭을 맞춘다).
ASCII_GLYPHS = Glyphs(
    tool_prefix="(*)",
    ellipsis="...",
    checkmark="[OK]",
...
# [해설] 프로세스 단위 캐시들. 테스트용으로 `reset_glyphs_cache`, `reset_langsmith_url_cache`가 비운다.
...
# [해설] LangSmith 프로젝트 URL 조회 제한 시간(초). `fetch_langsmith_project_url_or_raise`의 스레드 join timeout으로 쓰인다.
...
# [해설] 진단용 SDK(`deepagents`) 버전. `extras_info.resolve_sdk_version`에 위임하며, 해석 실패면 None. `build_stream_config` 메타데이터에 사용.
...
# [해설] editable 설치면 `1.2.3+editable` 형태로 PEP 440 local 세그먼트를 붙인다(SDK 쪽 `lc_versions` 표기와 통일).
...
# [해설] PEP 610 `direct_url.json`을 한 번 읽어 editable 여부와 소스 경로를 캐시한다. 배너의 "(local)" 표시와 트레이스 메타데이터 `editable`에 사용.
...
# [해설] editable 소스 경로(`~` 축약)만 반환하는 래퍼.
...
# [해설][흐름] 1) 접두 env `DEEPAGENTS_CODE_UI_CHARSET_MODE` → 비접두 `UI_CHARSET_MODE` → 2) stdout 인코딩에 utf 포함? → 3) LANG/LC_ALL에 utf 포함?
# [해설][주의] 여기서는 접두 변수가 "존재"하면 채택하는 규칙을 직접 구현한다(resolver 미사용).
...
# [해설] TUI 위젯들이 호출하는 공개 API. 감지된 모드에 맞는 글리프 세트를 캐시해 반환.
...
# [해설] 위젯이 문자셋 분기를 간단히 하도록 제공하는 편의 함수.
...
# [해설] 입력창의 "줄바꿈" 단축키 라벨. kitty 키보드 프로토콜 지원 시 Shift+Enter, 아니면 macOS Option+Enter / 그 외 Ctrl+J.
...
# [해설] 시작 스플래시 배너(f-string으로 버전 삽입). `get_banner`가 문자셋에 맞춰 선택한다.
_UNICODE_BANNER = f"""
██████╗  ███████╗ ███████╗ ██████╗    ▄▓▓▄
██╔══██╗ ██╔════╝ ██╔════╝ ██╔══██╗  ▓•███▙
██║  ██║ █████╗   █████╗   ██████╔╝  ░▀▀████▙▖
...
# [해설] 배너 선택 + `DEEPAGENTS_CODE_HIDE_SPLASH_VERSION`이면 버전 제거, editable 설치면 "(local)" 표시.
def get_banner() -> str:
...
# [해설] 도구 인자 표시 길이 제한(문자). `tool_display.truncate_value`에서 사용.
MAX_ARG_LENGTH = 150
...
# [해설] cwd별 git 브랜치 캐시. 세션 중 브랜치 변경은 반영되지 않는다(커밋 SHA는 매번 조회하는 것과 대조).
...
# [해설] 현재 cwd의 git 브랜치를 캐시와 함께 반환. 트레이스 메타데이터 `git_branch`와 상태바 표시용 (추정).
...
# [해설] HEAD 커밋 SHA. 턴마다 달라질 수 있어 캐시하지 않는다(트레이스에 턴 당시 커밋 기록).
...
# [해설] `origin` 리모트 URL을 파싱한 (url, provider, name) 메타데이터를 cwd별 캐시.
...
# [해설] 아래 4개 상수는 LangSmith "coding-agent-v1" 트레이스 메타데이터 계약의 고정 식별자다.
...
# [해설] coding-agent-v1 계약 메타데이터 블록을 만든다: 식별 키 + 버전 + 턴 번호 + repo/git/cwd + user_id/sandbox_type(알 수 없는 키는 생략).
# [해설][설계] 서브에이전트/루트 전용 키(approval_policy, ls_subagent_*)는 넣지 않는다 — LangGraph 메타데이터가 모든 하위 run에 전파되어 계약 위반이 되기 때문.
# [해설] 호출자: `build_stream_config`.
...
# [해설][흐름] 클라이언트가 매 턴 LangGraph 서버로 스트림을 요청할 때 넘기는 `RunnableConfig`를 조립한다(`configurable.thread_id`, `metadata`, `recursion_limit`).
# [해설] 호출자: TUI/headless 실행 경로(`textual_adapter`/`non_interactive` 등, 추정). 관련: analysis/01(클라이언트↔서버 스트리밍).
def build_stream_config(
    ...
    *,
    ...
    """
    # [해설][흐름] 1) cwd 결정 → 2) 계약 메타데이터 → 3) 진단용 키(experimental, auto_approve(YOLO), skill, TERM_PROGRAM, editable, lc_versions, SDK 버전, agent 이름)
    ...
    # [해설][흐름] 4) config 조립. recursion limit은 매 run마다 보내야 설정이 반영된다.
    ...
    # [해설] `resolve_recursion_limit`: managed > CLI > env > config.toml > LANGGRAPH_DEFAULT_RECURSION_LIMIT 순 해석, 없으면 None → 서버 기본값(analysis/03 참고).
...
# [해설] "모든 셸 명령 허용" 센티널 타입. `is` 비교 대신 `isinstance`로 판별해 복사/직렬화 후에도 의미가 유지된다.
...
# [해설] `--shell-allow-list=all`의 결과값. `is_shell_command_allowed`가 isinstance로 검사해 즉시 허용한다.
...
# [해설] `--shell-allow-list` / `DEEPAGENTS_CODE_SHELL_ALLOW_LIST` 문자열 파싱: "all" → 센티널, "recommended" → `RECOMMENDED_SAFE_SHELL_COMMANDS`, 그 외 쉼표 분리.
# [해설] 관련: analysis/04-approval-hitl-security.md(셸 승인 우회 목록).
...
# [해설] 이미 분리된 항목 리스트 파싱(TOML 배열 등). "all"은 단독이어야 하고 "recommended"는 커스텀 항목과 병합(중복 제거) 가능.
def parse_shell_allow_list_items(items: list[str]) -> list[str] | None:
    ...
    """
    # [해설][흐름] 1) 공백 항목 제거 → 2) 단독 "all"이면 센티널 → 3) "all" 혼용은 ValueError → 4) "recommended" 확장 → 5) 순서 유지 중복 제거.
...
# [해설] PTC(Programmatic Tool Calling: 인터프리터 REPL 안에서 도구 호출) "safe" 프리셋 — 원래 HITL 대상이 아닌 읽기 전용 도구만 포함.
# [해설][주의] PTC는 `interrupt_on`을 우회하므로 execute/write/네트워크/task 도구를 넣으면 승인 없이 실행되는 권한 상승이 된다(analysis/04 참고).
...
# [해설] "all"은 agent 빌드 시 실제 도구 목록으로 확장되며, auto_approve가 아닐 때는 `interpreter_ptc_acknowledge_unsafe=True`가 필요하다(docstring 기준).
...
# [해설] TOML/CLI의 `interpreter_ptc` 값을 정규형(False | "safe" | "all" | 도구명 리스트)으로 강제 변환. True는 모호하므로 거부.
# [해설] 호출자: config_manifest 또는 agent 빌드 경로 (추정).
...
# [해설] `[retries.<provider>]` 테이블 1개의 검증된 값: max_retries(예산), param(프로바이더 SDK의 재시도 kwarg 이름).
@dataclass(frozen=True)
...
# [해설] `[retries]` 전체 파싱 결과 + 사용자에게 보여줄 진단 메시지(warnings).
@dataclass(frozen=True)
class _RetryConfig:
...
# [해설] 정수 >= 0만 허용(bool은 int 하위 타입이라 명시적으로 제외).
...
# [해설] param은 kwargs 이름으로 쓰이므로 파이썬 식별자이면서 예약어가 아니어야 한다.
...
# [해설] `[retries]` 테이블 순수 파싱(로깅 부작용 없음). 전역 `max_retries` + 프로바이더별 하위 테이블. 알 수 없는 키는 진단만 남기고 무시.
# [해설][설계] 알 수 없는 프로바이더 테이블도 버리지 않고 유지한다(langchain 레지스트리가 실제로 그 이름으로 빌드할 수 있으므로).
...
# [해설][흐름] managed + user `config.toml`을 병합(`configuration.service.get_config_sources().merged()`)해 `[retries]`를 읽는다.
# [해설] 사용자 파일 파손/managed 정책 미적용도 진단 메시지로 수집. 알려진 프로바이더 = model_config 레지스트리들 ∪ `[models.providers]` 키.
# [해설] 호출자: `create_model`, `collect_retry_config_startup`.
...
# [해설] 프로바이더별 max_retries가 있으면 우선, 없으면 전역 값.
...
# [해설] "재시도 kwarg 모름" 경고를 프로바이더당 한 번만 내기 위한 집합(create_model은 서브에이전트·/model마다 반복 호출됨).
_warned_unknown_retry_providers: set[str] = set()
"""Providers already reported as having no identifiable retry control."""


# [해설] 프로바이더 SDK 자체 재시도를 끄기 위한 kwarg 이름 결정: `[retries.<p>].param` > `model_config.RETRY_PARAM_BY_PROVIDER` > kwargs에 이미 `max_retries`가 있으면 그것.
...
# [해설][설계] 재시도 소유권은 dcode 미들웨어(`model_retry.CodeModelRetryMiddleware`)에 있으므로 SDK 재시도를 0(google_genai 등은 레지스트리 값)으로 강제해 중첩 재시도 곱셈을 막는다.
# [해설] 사용자가 kwargs로 다른 값을 줬으면 무시하고 경고. 반환: `{param: disable_value}` 또는 `{}`.
def _provider_retry_disable_kwargs(
    ...
    if retry_param is None:
        ...
        # [해설] 레지스트리에 None으로 등록된 프로바이더 = "재시도 kwarg가 없음을 확인함" → 경고 없이 빈 dict.
    ...
    # [해설] 끄는 값은 기본 0. 일부 프로바이더는 다른 값을 요구한다(예: google_genai=1, `model_config.py` 레지스트리 — analysis/03).
...
# [해설] 설정이 전혀 없을 때의 모델 노드 재시도 횟수(첫 호출 제외). `model_retry`가 재수출한다. 공식 문서 `config-file.md` "Retries" 기본값 5와 일치.
...
# [해설] `create_model`이 모델 객체에 `object.__setattr__`로 박아 두는 재시도 예산 속성명. 런타임 `/model` 교체 후에도 미들웨어가 요청마다 읽는다.
MODEL_RETRIES_ATTR = "_deepagents_model_retries"
"""Private model attribute carrying the budget resolved when it was built."""


# [해설] 재시도 예산 우선순위: `--max-retries` > `[retries.<provider>].max_retries` > `[retries].max_retries` > 5.
...
# [해설] 시작 시 진단 표시용: 재시도 설정 경고와 "dcode가 덮어쓸 kwarg 이름" 집합을 반환(예: `--model-params`에 준 max_retries가 무시될 것임을 알림). 호출자: main/app 시작 경로 (추정).
...
# [해설] 상대 경로로 적힌 extra skill 루트를 어느 프로젝트 기준으로 해석할지 담는 ContextVar(/reload·cwd 전환 시 대상 경로 기준으로 검증하기 위함).
...
# [해설] 블록 동안 extra skills 상대 경로 기준 디렉터리를 바인딩.
@contextmanager
...
# [해설] `~` 확장 → (상대 경로면) 바인딩된 기준 경로와 결합 → symlink resolve.
...
# [해설] 스킬 경로 containment 허용 목록 확장(탐색 위치 추가가 아님). env `DEEPAGENTS_CODE_EXTRA_SKILLS_DIRS`가 있으면 config.toml 값은 무시.
# [해설] 관련: analysis/06-memory-skills.md(`load_skill_content`의 경로 검증).
...
# [해설] managed 정책 때문에 /reload가 막혔을 때 변경 목록 첫 항목에 붙는 접두 문구. 판정은 문자열 비교 대신 `managed_reload_block`을 쓰도록 한다.
...
# [해설] reload 결과 목록에서 managed 차단 알림을 찾는다(위치가 아니라 전체 스캔). 호출자: app.py의 /reload·재시작 경로들(docstring상 4곳).
...
# [해설][설계] /reload·cwd 전환 시 `CredentialsSnapshot`에서 새로 고치는 필드. 모델 메타데이터(`RuntimeState`)와 사용자 원래 LangSmith 프로젝트는 제외.
_RELOADABLE_FIELDS = (
    "openai_api_key",
    "anthropic_api_key",
    "google_api_key",
...
# [해설] 변경 보고에서 값을 마스킹("set"/"unset")할 필드 — 이름이 `_api_key`로 끝나면 자동 포함.
...
# [해설] 자격증명 스냅샷이 아니라 ConfigResolver에서 오는 값 중 reload 보고에 포함되는 것(shell allow-list, extra skills dirs).
...
# [해설] 마지막으로 "수용된" resolver 기반 값 기억(다음 reload의 비교 기준). 락으로 보호.
...
# [해설][설계] rank 350의 "reload 보존 계층" provider. CLI(300) 아래, ENV(400) 위에 끼워져, 새 generation이 재현하지 못하는 값(env에서만 오던 값 등)을 프로세스 수명 동안 유지한다.
# [해설] `configuration/resolver.py`의 provider 프로토콜(durable/get/status/reload)을 구조적으로 구현. 관련: analysis/03 "D. /reload".
...
class _ReloadOverrideProvider:
    ...
    # [해설] 보존 값이 있으면 Found, 없으면 Unset(하위 rank로 넘어감).
    ...
    # [해설] resolver generation 전진 시 호출되지만 의도적으로 아무 것도 하지 않는다(보존 값은 `replace`로만 바뀐다).
...
# [해설] 공유 resolver(`get_config_resolver`)에 보존 계층이 없으면 설치 후 반환.
...
# [해설][흐름] reload 후, 보존 계층을 제외한 resolver 해석값이 이번 reload에서 수용한 값과 다르면 그 값을 rank 350에 보존한다(예: env 파싱 결과가 resolver와 다른 경우).
...
# [해설] 기억된 값의 복사본 반환(reload 비교의 previous).
...
# [해설] 현재 resolver로 두 옵션을 한 lock 안에서(`resolve_options`) 해석해 스냅샷. manifest에 옵션이 없으면 RuntimeError.
...
# [해설] 현재 프로세스의 활성 모델 메타데이터(이름/프로바이더/컨텍스트 한도/미지원 모달리티). `ModelResult.apply_to_runtime_state`가 갱신하고 UI 상태바 등이 읽는다 (추정).
@dataclass
class RuntimeState:
...
# [해설][설계] 자격증명·프로젝트 컨텍스트의 불변 "한 세대" 스냅샷. /reload는 새 스냅샷을 만들어 참조 한 번으로 교체(부분 적용 방지).
# [해설][주의] 여기 담기는 키는 env(접두 우선)에서 읽은 값뿐이다. `/auth` 저장 키(auth.json)는 반영되지 않으므로, 실제 모델 생성 키 판단은 `create_model`의 `resolve_provider_credential`이 한다.
...
# [해설] `Credentials.__getattr__/__setattr__`가 스냅샷으로 위임할 필드 이름 집합.
_CREDENTIAL_FIELDS = frozenset(CredentialsSnapshot.__dataclass_fields__)


# [해설] 활성 `CredentialsSnapshot`의 안정적인 소유자. 모듈 전역 `credentials`(lazy, `_get_credentials`)가 이 타입.
# [해설] 필드 읽기는 스냅샷으로 위임, 필드 쓰기는 `dataclass_replace`로 새 스냅샷을 publish(기존 코드 호환). 호출자: app.py, agent.py, main.py 등.
class Credentials:
    ...
    # [해설] 정상 속성 조회 실패 시에만 호출되는 훅 — 자격증명 필드면 활성 스냅샷에서 읽는다.
    ...
    # [해설] 자격증명 필드 대입은 스냅샷 교체로 변환(원자적). 그 외 속성(`_active` 등)은 일반 대입.
    ...
    # [해설] env(기본: `active_environment()`)에서 스냅샷을 계산만 하고 publish하지 않는다. 서버 워크스페이스별 스냅샷 계산에도 쓰인다.
    # [해설] 키는 `_resolve_env_var_from`으로 접두 env 우선 조회. `project_root`는 git 루트 탐색(`project_utils.find_project_root`).
    @classmethod
    ...
    # [해설] 최초 생성 경로(`_get_credentials`): 스냅샷 생성 + 보존 계층 비우기 + 현재 resolver 값 기억.
    @classmethod
    ...
    # [해설][흐름] /reload(및 미리보기)의 핵심 계산: env 매핑과 managed/user 설정에서 reload 대상 값을 모두 해석한다. 반환: (새 값 dict, 알림 문자열|None).
    # [해설] 단계: 1) managed 스냅샷 건강성 확인(실패 시 이전 값 유지+차단 알림) 2) resolver generation 전진 3) config.toml 파손 알림
    # [해설] 4) shell allow-list 해석 5) project_root 6) extra skills dirs 7) 자격증명 필드.
    @staticmethod
    def _reload_values(
        *,
        start_path: Path | None,
        ...
        # [해설][흐름] 1) managed 정책 스냅샷을 대상 프로젝트 기준으로 검증. `ManagedConfigError`면 fail-closed(이전 값 + `MANAGED_RELOAD_BLOCKED_PREFIX` 알림).
        try:
        ...
        # [해설][흐름] 2) 방금 검증한 managed 스냅샷으로 공유 resolver generation을 한 번에 전진(`get_config_resolver(refresh_managed=..., managed_snapshot=...)`).
        ...
        # [해설][흐름] 3) 사용자 config.toml(rank 500)이 unusable이면 "Kept previous config.toml" 알림 생성(이전 generation 유지 사실을 사용자에게 알림).
        ...
        # [해설][흐름] 4) shell allow-list: 먼저 env(`DEEPAGENTS_CODE_SHELL_ALLOW_LIST`) 파싱값을 후보로 두고, 잘못된 값이면 이전 값 유지.
        try:
        ...
        # [해설] 미리보기(refresh_managed=False)는 공유 resolver를 바꿀 수 없으므로 디스크의 config.toml을 새로 읽은 "후보 resolver"를 따로 만든다.
        ...
        # [해설] resolver 해석(보존 계층 350 제외) 결과가 managed/CLI/config.toml에서 결정됐다면 env 파싱값 대신 그 값을 채택한다.
        ...
        # [해설][흐름] 6) extra skills dirs: managed가 결정했으면 그 값, 아니면 user tier 값, env가 있으면 env 우선(단 managed 결정 시 env 무시).
        try:
            with _use_extra_skills_path_base(start_path):
        ...
        # [해설][흐름] 7) API 키/GCP/LangSmith 프로젝트는 env 매핑에서 접두 우선으로 다시 읽는다.
        return {
    ...
    # [해설] 이전/새 값 비교해 "field: old -> new" 목록 생성. API 키는 set/unset으로 마스킹.
    @staticmethod
    ...
    # [해설] /reload 드라이런: `_preview_dotenv_environ`으로 os.environ을 바꾸지 않고 계산, managed 재읽기 금지(refresh_managed=False). 호출자: 재시작/cwd 전환 확인 UI (추정).
    ...
    # [해설][흐름] `/reload` 본체(클라이언트 전용). app.py가 스레드에서 호출한다(analysis/03 "D. /reload").
    # [해설] 단계: 1) previous 수집·보존 계층 설치 2) LangSmith launch env 복원 3) dotenv 주입값 제거 후 재로드 4) 저장 LangSmith 인증 재적용
    # [해설] 5) `_reload_values` 6) 보존 계층 동기화 7) LANGSMITH_PROJECT 재동기화 8) 진단 dedup 리셋 9) managed 차단이 아니면 새 스냅샷 publish.
    def reload_from_environment(self, *, start_path: Path | None = None) -> list[str]:
        ...
        # [해설][흐름] 2) 캐리어가 있으면(서버에서 띄워진 in-process 모드 등, 추정) launch LangSmith 값을 캐리어 기준으로 갱신. 깨졌으면 env를 건드리지 않고 알림.
        ...
        # [해설][흐름] 3) refresh_loaded=True: 이전 dotenv 주입값을 걷고 (새 cwd 기준) 다시 로드 — cwd 전환 시 새 프로젝트 `.env` 반영.
        _load_dotenv(
            start_path=start_path,
            refresh_loaded=True,
            capture_user_langsmith=True,
        ...
        # [해설][흐름] 4) 저장된 서비스 키/트레이싱 설정 재적용 → 5) 새 값 계산.
        # [해설][주의] 서버 subprocess로는 여기서 전달되지 않는다. 서버 env는 spawn할 때마다 `_bootstrap_state`에서 재인코딩된다(위 주석, analysis/03 한계).
        apply_stored_langsmith_auth()
        refreshed, blocked = self._reload_values(
            start_path=start_path,
            env=dict(os.environ),
        ...
        # [해설][흐름] 6) 새 스냅샷을 만들어 두고(아직 publish 전), resolver 값 기억 + 보존 계층 동기화.
        ...
        # [해설][흐름] 7) 에이전트용 프로젝트 override가 바뀌면 LANGSMITH_PROJECT를 갱신, 해제됐으면 사용자 원래 값 또는 기본값으로 복귀.
        ...
        # [해설][흐름] 9) managed가 막았으면 스냅샷을 publish하지 않는다(단, 위의 os.environ/dotenv 변경은 이미 일어났다는 점에 주의 (추정)).
        if managed_reload_block([blocked] if blocked else []) is None:
            self._active = replacement
        return [blocked, *changes] if blocked else changes

    # [해설] 아래 has_* 프로퍼티는 호환용으로 활성 스냅샷에 위임한다.
    @property
    def has_anthropic(self) -> bool:
        """Check if Anthropic API key is configured."""
...
# [해설][설계] 셸 allow-list 우회 방지용 금지 부분문자열(명령 치환, 리다이렉트, 개행, here-doc 등). allow-list에 있는 명령이어도 이 패턴이 있으면 승인 필요.
# [해설] 사용처: `contains_dangerous_patterns` → `is_shell_command_allowed`. 관련: analysis/04.
...
# [해설] `--shell-allow-list recommended`가 확장되는 읽기 전용 명령 목록. 셸/에디터/인터프리터/네트워크 도구는 GTFOBins 위험 때문에 제외.
# [해설][주의] 첫 토큰(명령 이름)만 검사하므로 인자로 인한 위험(예: `cat`으로 민감 파일 읽기)은 막지 않는다 (추정: 설계상 읽기는 허용).
...
# [해설] 금지 부분문자열 + 정규식(`$VAR` 형태 변수 확장, `&&`가 아닌 단독 `&` 백그라운드 실행) 검사.
...
# [해설][흐름] 셸 명령 자동 승인 판정(보안 핵심). 호출자: 비대화형 모드/승인 미들웨어의 execute 게이트 (analysis/04).
# [해설] 1) 빈 입력/목록 없음 → False 2) ALL 센티널 → True(패턴 검사도 생략) 3) 위험 패턴 → False 4) `&&`, `||`, `|`, `;`로 분할해 각 세그먼트 첫 토큰이 모두 allow-list에 있어야 True.
def is_shell_command_allowed(command: str, allow_list: list[str] | None) -> bool:
    ...
    # [해설][주의] 세그먼트 분할은 따옴표를 고려하지 않는 정규식이다. 따옴표 안 `|`로 쪼개져도 shlex 실패 시 False(보수적)로 떨어지거나, 더 엄격해지는 방향이라 우회로 이어지진 않는다 (추정).
...
# [해설] UI·스트림 메타데이터에 표시할 LangSmith 프로젝트명. 트레이싱이 켜져 있고 업로드 가능할 때만 반환(아래 본문 참고).
def get_langsmith_project_name() -> str | None:
    ...
    """
    # [해설][흐름] 1) 키(접두 우선)와 트레이싱 플래그가 모두 있어야 한다 → 2) 스코프 env면 스냅샷에서, 아니면 전역 `credentials`에서 에이전트용 override → 3) LANGSMITH_PROJECT → 4) 기본값.
    from deepagents_code.config_manifest import LANGSMITH_PROJECT_DEFAULT
...
# [해설] `/trace`가 키를 못 찾았을 때 원인을 구분하기 위한 결과 타입: 빈 접두 override가 키를 가림 / auth 저장소 읽기 실패 / 해당 없음.
@dataclass(frozen=True)
...
# [해설] 흔한 실수 진단: `DEEPAGENTS_CODE_LANGSMITH_API_KEY=`(빈 값)가 표준 env 키와 `/auth` 저장 키를 모두 가리는 경우 그 변수 이름을 알려준다.
# [해설] 호출자: TUI `/trace` 명령 (추정: app.py 명령 핸들러).
def langsmith_key_shadowed_by_empty_override() -> LangsmithShadowResult:
    ...
    # [해설][흐름] 키별로: 접두 override가 비어 있을 때만 검사 → 표준 변수에 값이 있으면 그 override가 범인 → LANGSMITH_API_KEY는 저장 키 존재도 확인.
...
# [해설] `tracing.langsmith_redact` 옵션(ConfigResolver 경유, 기본 True). 트레이스 비밀값 마스킹 여부.
# [해설] 아래 is_* 함수들은 모두 같은 패턴: manifest 옵션 조회 → 공유 resolver → 진단 출력(`_emit_ranked_diagnostics`) → bool.
...
# [해설] `memory.auto_save`(기본 True). 끄면 메모리는 로드하되 에이전트에게 자동 저장하지 말라고 지시한다(analysis/06).
...
# [해설] `startup.yolo_switcher`(기본 True). Shift+Tab 승인 모드 순환에 YOLO 포함 여부(analysis/04).
...
# [해설] `models.openai_prompt_cache_key`(기본 True). `ConfigurableModelMiddleware`가 thread id를 OpenAI `prompt_cache_key`로 주입할지 여부.
...
# [해설][흐름] Auto 승인 분류기 모델 해석(보안 관련). 반환 (spec|None=메인 모델 상속, problem 메시지|None).
# [해설] 순서: 1) managed가 결정했으면 그 값(빈 값은 상속+경고) 2) env가 "존재하지만 빈 값"이면 명시적 상속(config.toml보다 우선) 3) resolver 값 4) TOML 타입 오류 진단.
# [해설][설계] 설정이 무시되면 "에이전트가 자기 행동을 채점"하는 상태가 되므로 사용자에게 보이는 문구로 알린다. 관련: analysis/04, 공식 `config-file.md` Auto classifier.
def resolve_auto_classifier_model_with_problem() -> tuple[str | None, str | None]:
    ...
    # [해설] 공유 resolution 결과에서 user tier(config.toml) 값만 꺼낸다 — 빈 env가 무엇을 덮었는지 경고에 명시하기 위함.
...
# [해설] 분류기 모델 미설정 시 메인 모델 프로바이더별 기본 분류기(더 가벼운 모델). 여기 없는 프로바이더는 None → 메인 모델 상속.
...
# [해설] 메인 프로바이더를 안 뒤 분류기를 확정: 명시값 > 설정값 > 프로바이더 기본값. 설정 문제 또는 `models.allowed` 정책 위반이면 `INHERIT_CLASSIFIER_MODEL` 센티널(메인 모델 사용).
...
# [해설] `goals.auto_accept_criteria`: Auto 모드에서 생성된 goal 기준을 검토 없이 적용할지. manifest 항목이 없으면 False로 fail-closed(analysis/05).
...
# [해설][흐름] LangSmith 비밀값 anonymizer가 달린 Client를 전역 설치한다(보안 통제, fail-closed).
# [해설] 1) 트레이싱 off거나 업로드 대상 없음 → 할 일 없음 2) redaction 옵션 off → 경고 후 False 3) 키/엔드포인트로 `langsmith.Client(anonymizer=...)` 생성·`configure`
# [해설] 실패 시 `_fail_closed_disable_tracing`으로 트레이싱 자체를 끈다. 호출자: `apply_stored_langsmith_auth`(부트스트랩·/auth).
def configure_langsmith_secret_redaction() -> bool:
    ...
    # [해설] fail-closed 경계 시작: 이 블록의 어떤 예외든 트레이싱 비활성화로 귀결.
    try:
...
# [해설] SDK 전역 스위치 `configure(enabled=False)`가 1차 통제, 그것도 실패하면 트레이싱 env(접두 포함)를 제거하는 최후 수단.
# [해설][주의] SDK env 조회는 캐시되므로 env 제거만으로는 완전하지 않다(docstring 명시).
...
# [해설] `DEEPAGENTS_CODE_LANGSMITH_REPLICA_PROJECTS`(쉼표 구분)를 중복 제거 리스트로. 에이전트 트레이스 이중 기록용.
...
# [해설][설계] 에이전트 run은 LangGraph 서버 subprocess에서 생성되므로 클라이언트 콜백으로는 미러링 불가 → 서버의 replica 경로(요청에 project 전달)를 쓰며, 서버는 1개만 지원한다.
...
# [해설] 트레이싱 활성 판정(접두 인식). 브리지되는 2개 플래그는 접두 우선, 나머지 플래그는 표준 이름만 인정 — 런타임 브리지 동작(`_apply_prefixed_langsmith_env`)과 일치.
...
# [해설] "설정 안 됨"과 "명시적으로 끔"을 구분(`dcode doctor` 표시용). 빈 접두 플래그가 truthy 표준 플래그를 가리는 경우도 끔으로 본다.
...
# [해설] 활성 env(스코프 또는 os.environ) 기준 트레이싱 활성 여부.
def _tracing_enabled() -> bool:
    """Return whether tracing is (or will be) enabled, prefix-aware."""
    return _tracing_enabled_from(active_environment())


# [해설] env(접두 인식) API 키 또는 LangSmith 프로필 자격증명 존재 여부.
...
# [해설] `LANGSMITH_RUNS_ENDPOINTS` JSON(리스트 또는 dict 형태)에서 레플리카 적재 URL을 SDK와 같은 규칙으로 추출. 잘못된 항목은 무시.
...
# [해설] 업로드 가능 여부 = 자격증명 or 커스텀 엔드포인트 or 레플리카 엔드포인트. redaction 필요성 판단 기준.
...
# [해설] 커스텀 트레이싱 엔드포인트: SDK 순서(env > 프로필)와 동일. env가 SDK 기본 US URL이면 None(프로필로 넘어가지 않음).
...
# [해설] 진단용 프로젝트명 해석(부트스트랩 없이). 접두 값이 비어 있으면 건너뛴다 — `resolve_env_var`의 "빈 접두가 가림" 규칙과 다름에 주의.
...
# [해설] `dcode doctor`는 부트스트랩 전에 돌기 때문에, os.environ을 바꾸지 않고 dotenv를 적용한 미리보기 env를 만든다.
...
# [해설] `dcode doctor`에 표시할 트레이싱 설정 스냅샷. 비밀값은 담지 않고 존재/식별 정보만.
@dataclass(frozen=True)
class TracingStatus:
    ...
    # [해설] 불변식: enabled와 explicitly_disabled가 동시에 True일 수 없다.
...
# [해설] 오프라인 트레이싱 진단 요약(네트워크 호출 없음). 호출자: `dcode doctor` (analysis/01·09 참고, 추정).
...
# [해설] LangSmith 프로젝트 URL 조회 실패의 예외 계층. `/trace`가 원인별 메시지를 보여줄 수 있게 타입으로 구분한다.
...
# [해설] SDK의 404 예외인지 판정(SDK 미설치면 False).
...
# [해설] 프로젝트 URL + `/t/<thread_id>` + utm 태그로 스레드 링크 생성.
...
# [해설][흐름] 프로젝트 URL 조회(네트워크). 1) 캐시 hit → 반환 2) langsmith import 3) 데몬 스레드에서 `Client.read_project` 4) 최대 2초 대기 후 timeout 예외
# [해설] 5) 스레드 오류를 404/기타로 분류 6) 성공 시 캐시.
# [해설][주의] timeout 후에도 데몬 스레드는 계속 돌 수 있다(join하지 않음). 호출 스레드만 제한 시간 내 풀린다.
def fetch_langsmith_project_url_or_raise(project_name: str) -> str:
    ...
    # [해설] 워커 스레드 본체. `Client()`는 접두 env를 모르므로 api_key를 명시적으로 넘긴다.
...
# [해설] 실패 시 None을 돌려주는 호환 래퍼(상태 배너, 비대화형 출력용).
...
# [해설] 프로젝트명 해석 + URL 조회 + 스레드 링크 조립(블로킹 최대 2초).
...
# [해설] 캐시에 있을 때만 링크를 만드는 논블로킹 버전(일시적 UI 표시용).
...
# [해설] 패키지에 포함된 `default_agent_prompt.md`(기본 시스템 프롬프트)를 읽는다. AGENTS.md 장기 메모리는 미들웨어가 별도로 처리(analysis/02·06).
...
# [해설] Bedrock 교차 리전 inference profile 접두(`us.` 등). `_is_bedrock_model_id`가 벗겨낸 뒤 vendor 토큰을 본다.
...
# [해설] bare 모델 이름이 Bedrock ID(`[region.]vendor.model[:ver]`)인지 구조로 판정: 첫 점 앞이 영숫자 vendor 토큰이면 True.
...
# [해설] 프로바이더 추론 시 사용할 자격증명 소스: 스코프 env면 즉석 스냅샷, 아니면 전역 `credentials`.
...
# [해설][흐름] `provider:` 없이 준 bare 모델 이름에서 프로바이더를 추론(LangChain `_attempt_infer_model_provider`의 부분 복제).
# [해설] `init_chat_model` 호출 전에 프로바이더별 kwargs와 조기 자격증명 검사가 필요하기 때문. 호출자: `create_model`, `/model` 확인 UI, 상태바 등.
# [해설][주의] 규칙 순서가 의미를 가진다: gpt/o-시리즈 → Bedrock 구조 → cohere/mistral/deepseek/xai/perplexity → claude/gemini(자격증명으로 Vertex 분기) → nvidia → fireworks.
def detect_provider(model_name: str) -> str | None:
    ...
    **before** calling `init_chat_model` in order to:
    ...
    # [해설] claude: Anthropic 키가 없고 Vertex(GCP 프로젝트 O, Google API 키 X)면 `google_anthropic_vertex`. 스냅샷은 env만 보므로 `/auth` 저장 키는 여기서 고려되지 않는다 (추정).
...
# [해설] `models.allowed` 한 항목을 후보 spec 목록으로 확장: 정확한 spec은 그대로, `provider:*`는 발견된 모델 목록(`get_discovered_models`)으로.
...
# [해설][흐름] 시작 모델 자동 선택(`--model`이 없을 때 `create_model`이 호출). 반환: `provider:model`.
# [해설] 1) `[models].default` 2) `[models].recent` (둘 다 allowlist 밖이면 경고 후 건너뜀) 3) allowlist가 있으면 자격증명이 MISSING 아닌 첫 후보(없으면 예외, 4단계로 가지 않음)
# [해설] 4) openai → anthropic → google_genai 자격증명 순으로 하드코딩 기본 모델.
# [해설][문서 불일치] 공식 `docs_official/code/providers.md` "Model resolution order"는 env 자동감지에 `GOOGLE_CLOUD_PROJECT`도 포함한다고 하지만, 이 코드의 4단계는 3개 프로바이더만 본다(analysis/03).
def _get_default_model_spec() -> str:
    ...
    # [해설][흐름] 3) 관리자/사용자 `models.allowed` 정책 활성. 빈 목록이면 전면 거부(`ModelNotAllowedError`, 자격증명 추가로 해결 불가).
    ...
    # [해설][흐름] 4) 자격증명 자동감지. `get_provider_auth_status`는 `/auth` 저장 키와 env를 모두 본다(model_config.py). UNKNOWN은 선택하지 않는다.
    # [해설][주의] 기본 모델 이름은 코드에 하드코딩되어 있어 릴리스마다 바뀐다.
    ...
    # [해설] 어떤 자격증명도 없으면 `NoCredentialsConfiguredError` — 호출자(TUI)는 이를 잡아 시작을 미루고 자격증명 입력을 유도할 수 있다.
    msg = (
...
# [해설] OpenRouter 앱 attribution 헤더 기본값(HTTP-Referer, X-Title, 카테고리). `_ensure_cli_openrouter_profile_registered`가 SDK profile에 스택 등록한다.
...
# [해설] LangChain Bedrock 생성자 인자명으로 바꾼 AWS env 테이블(profile 인자명이 boto3와 다름).
...
# [해설] "생성자 인자 → 후보 env 이름들" 테이블을 lookup 함수로 해석해 명시적 kwargs로 변환(첫 non-empty 값 채택).
...
# [해설][설계] SDK가 원래 os.environ에서 직접 읽는 값들. 워크스페이스 스코프 런타임에서는 os.environ이 아닌 스냅샷을 써야 하므로 명시적 kwargs로 넣어준다(`_apply_provider_sdk_environment`).
...
# [해설] OpenRouter profile의 `init_kwargs_factory` — app_url/app_title 반환.
...
# [해설][SDK] `deepagents.profiles.provider.register_provider_profile`로 SDK 내장 openrouter profile 위에 앱 attribution을 additive 머지 등록(`libs/deepagents/deepagents/profiles/provider/provider_profiles.py`).
# [해설] 우선순위: SDK 기본 < 앱 profile < 호출자 kwargs(config.toml). 호출자: `create_model` (추정).
...
# [해설] Azure OpenAI 엔드포인트를 생성자 필드명으로 넘긴다(`base_url_env` 사용자 override 존중).
def _apply_azure_sdk_endpoint(kwargs: dict[str, Any]) -> None:
    ...
    # [해설] 같은 엔드포인트가 base_url로도 들어와 있으면 base_url을 지우고 Azure 전용 필드 `azure_endpoint`로 옮긴다(명시적 base_url이 다르면 그대로 둔다).
...
# [해설] SDK가 os.environ에서 직접 읽던 값(AWS 리전/자격증명, Azure API 버전 등)을 `resolve_env_var`(접두 우선·스코프 인식)로 읽어 kwargs 기본값으로 채운다(명시 kwargs는 보존).
...
# [해설][흐름] config.toml `[models.providers.<p>]`의 params(모델별 shallow merge) + api_key + SDK env kwargs를 합쳐 생성자 kwargs를 만든다. 호출자: `create_model`.
# [해설] 1) `ModelConfig.get_effective_kwargs` 2) api_key_env(config 또는 `PROVIDER_API_KEY_ENV` 기본) → `resolve_env_var`(접두 env 우선) 3) Ollama 인증 헤더 4) SDK env 값.
def _get_provider_kwargs(
    ...
    # [해설][주의] 여기서 env(접두 우선)로 넣은 `api_key`는 이후 `create_model`에서 `/auth` 저장 키(`stored_credential`)가 있으면 덮어써진다.
    # [해설] google_anthropic_vertex는 키 슬롯에 GCP 프로젝트 id를 저장하므로 api_key로 넘기지 않는다.
    if api_key_env:
    ...
    # [해설] langchain-ollama는 api_key 인자가 없으므로 호스티드 Ollama 토큰을 `client_kwargs.headers.Authorization`에 Bearer로 주입(사용자 지정 헤더가 있으면 보존).
    if provider == "ollama":
...
# [해설][설계] 워크스페이스 스코프 경로(서버)에서는 `apply_stored_credentials`(os.environ 쓰기)를 건너뛰므로, 키와 엔드포인트의 짝을 맞추는 유일한 장치가 이 함수다.
# [해설] 게이트웨이용 키가 발급되지 않은 엔드포인트로 전송되는 것을 막는다. `--model-params`로 api_key를 명시했는지에 따라 분기.
def _apply_scoped_endpoint(
    ...
    if not (extra_kwargs and "api_key" in extra_kwargs):
        ...
        return
    # [해설] 명시 api_key + 명시 base_url → 사용자가 짝을 직접 지정했으므로 손대지 않음. 명시 api_key만 있으면 저장된 엔드포인트를 상속하지 않도록 제거.
...
# [해설][흐름] 스코프 경로에서 저장 키가 있을 때: config.toml base_url이 있으면 유지 → 저장 엔드포인트가 있으면 설정 → 없으면 상속된 base_url/커스텀 헤더 제거(anthropic은 기본 URL 명시).
# [해설] auth 저장소 읽기 실패 시에도 fail-closed로 상속 엔드포인트를 지운다. `model_config.apply_stored_credentials`의 os.environ 버전과 대응(analysis/03 설계 포인트 9).
...
# [해설] Claude-on-Vertex(`google_anthropic_vertex`)의 project/location 기본값 채우기. 스코프 경로에서는 저장소의 키 슬롯(=GCP 프로젝트)도 확인. location이 없으면 ModelConfigError.
...
# [해설] Anthropic "thinking block binding" 베타 헤더 값. Claude API(`anthropic` 프로바이더)에서만 적용, Bedrock/Vertex는 거부하므로 제외.
...
# [해설] block binding 기본 적용 대상 Claude 최소 메이저 버전(패밀리 우선 id `claude-opus-5` 형식에서 추출).
...
# [해설][설계] dcode는 턴 사이에 프롬프트 prefix를 재작성(예: 메모리/요약)하므로, 이전 prefix에 묶인 thinking 블록이 무효가 되어 턴이 실패할 수 있다.
# [해설] 그래서 `prefix_mismatch_behavior="drop_block"` 기본값 + (미설정 시) adaptive thinking + 베타 헤더를 넣는다. 사용자 명시값이 항상 우선. 호출자: `create_model`(bind_preserved_thinking=True일 때).
def _apply_anthropic_thinking_binding(
    ...
    if provider != "anthropic":
        return
    # [해설][흐름] 1) thinking 미설정이면 모델 버전 게이트(>=5) 통과 시 adaptive로 켬 2) 형식 이상은 경고 후 건너뜀 3) block_binding·betas 병합(중복 제거).
...
# [해설] 세션 `--model-params`의 `reasoning_effort`와 config.toml의 `reasoning` dict가 충돌하지 않게 하나의 `reasoning={..., effort}`로 합친다(openai/openai_codex 한정).
...
# [해설] config.toml `class_path = "module.path:ClassName"`로 지정한 커스텀 `BaseChatModel` 서브클래스를 import·검증·생성. 실패는 모두 ModelConfigError로 감싼다.
# [해설][주의] 설정 파일이 임의 모듈 import를 유발하므로 사용자 config.toml(신뢰 파일)에서만 와야 한다 — 프로젝트 config 계층은 없다(analysis/03).
...
# [해설] LangChain `init_chat_model`로 표준 프로바이더 모델 생성. ImportError를 "패키지 미설치(`/install` 힌트)"와 "설치됐지만 내부 import 실패"로 구분해 UI 친화 예외로 변환.
def _create_model_via_init(
    ...
    try:
        ...
        # [해설] 프로바이더 → pip 패키지명 예외 매핑(기본 규칙은 `langchain-<provider>`). `find_spec`으로 실제 설치 여부 판정.
        package_map = {
        ...
        raise ModelConfigError(msg) from e
    # [해설] provider가 비어 있고 init_chat_model도 추론 실패 → `UnknownProviderError`(UI가 문서 링크 표시).
...
# [해설] `create_model` 결과 묶음. 생성과 전역 상태 반영을 분리해 호출자가 커밋 시점을 결정한다(`apply_to_runtime_state`).
# [해설] `cli_max_retries`를 따로 들고 가는 이유: 다른 프로바이더로 재생성할 때 그 프로바이더의 설정 예산을 다시 해석하기 위함.
@dataclass(frozen=True)
class ModelResult:
    ...
    # [해설] 재시도 예산 불변식 검증(bool 금지, 음수 금지) — 수동 생성 시 실수를 조기 차단.
    def __post_init__(self) -> None:
    ...
    # [해설] 전역 `runtime_state`(lazy)에 모델 이름/프로바이더/컨텍스트 한도/모달리티 반영. 호출자: 클라이언트 시작·/model 전환 경로 (추정).
...
# [해설] 모델 `profile` dict에 override를 얕게 병합해 재할당(LangChain 모델 profile: max_input_tokens, tool_calling 등).
# [해설] config.toml 출처는 실패 시 경고, CLI `--profile-override` 출처는 raise_on_failure=True로 예외.
...
# [해설][흐름] dcode 모델 생성의 단일 진입점. 호출자: 클라이언트 시작(main/app), 서버 그래프 조립(agent.py), 런타임 교체(`configurable_model.ConfigurableModelMiddleware`),
# [해설] 서브에이전트·루브릭·Auto 분류기 모델 등. SDK `deepagents._models.resolve_model`은 쓰지 않고 인스턴스를 직접 만들어 넘긴다.
# [해설] 전체 단계: 1) 기본 spec 2) provider:model 파싱 3) 정책 게이트 4) 저장 자격증명 브리지·조회 5) 조기 자격증명 검사 6) kwargs 조립
# [해설] 7) SDK provider profile 8) --model-params·스코프 엔드포인트·reasoning/thinking 9) 재시도 예산·SDK 재시도 끄기 10) 생성 분기(codex/class_path/init_chat_model)
# [해설] 11) 비용 메타데이터·profile override·재시도 예산 스탬프·컨텍스트 한도 추출 → `ModelResult`. 관련: analysis/03 "B. 모델 생성".
def create_model(
    ...
    *,
    ...
    # [해설][흐름] 1) spec 미지정 → `_get_default_model_spec`(default → recent → allowlist → 자격증명 자동감지).
    if not model_spec:
        model_spec = _get_default_model_spec()

    # [해설][흐름] 2) 파싱 우선순위: config.toml에 정의된 프로바이더명 > Bedrock bare ID(`:0` 버전 접미사를 provider 구분자로 오인하지 않게) > 일반 `provider:model` > 선행 콜론 > bare 이름 추론.
    ...
    # [해설] 흔한 설정 실수 차단: Vertex의 Claude는 `google_anthropic_vertex` 프로바이더를 써야 한다.
    ...
    # [해설][흐름] 3) 정책 게이트(`ModelConfig.require_model_allowed`, `models.allowed`). 자격증명 브리지·profile pre_init·프로바이더 import보다 반드시 먼저(위치 자체가 보안 요건).
    ...
    # `UnboundLocalError`.
    # [해설][흐름] 4) 자격증명: 비스코프(클라이언트/단일 워크스페이스)면 `apply_stored_credentials`가 `/auth` 저장 키·엔드포인트를 표준 env에 복사.
    # [해설] 이어서 `resolve_provider_credential`로 "저장 키 우선" 자격증명을 얻는다(model_config.py).
    ...
    # [해설][흐름] 5) 조기 자격증명 검사 — SDK의 불투명한 인증 오류 대신 실행 가능한 안내(`MissingCredentialsError`). Vertex ADC 같은 암묵 인증 프로바이더는 제외.
    ...
    # [해설][흐름] 6) kwargs 조립: config params + env(접두 우선) api_key → 그 위에 저장 키가 있으면 api_key를 덮어씀.
    # [해설][문서 불일치] `docs_official/code/credentials.md` "Key resolution order"는 `DEEPAGENTS_CODE_` 접두 env > `/auth` 저장 키 > 표준 env라고 하지만,
    # [해설] 여기서는 `_get_provider_kwargs`가 접두 env로 넣은 api_key를 `stored_credential`(저장 키 우선)이 무조건 덮어쓴다 → 저장 키가 접두 env보다 우선(검증됨, analysis/03 대조표).
    ...
    # [해설][흐름] 7) [SDK] `deepagents.profiles.provider.apply_provider_profile`(`libs/deepagents/deepagents/profiles/provider/provider_profiles.py`): profile init_kwargs < 기존 kwargs, pre_init 훅 실행.
    # [해설] OpenRouter는 앱 attribution profile을 먼저 스택 등록한다. profile 훅의 임의 예외는 ModelConfigError로 변환.
    ...
    # [해설][흐름] 8) `--model-params`/`/model --model-params`(extra_kwargs)가 최우선으로 덮어씀 → 스코프 경로면 키/엔드포인트 짝 맞춤 → OpenAI reasoning 합성 → Anthropic thinking 바인딩.
    ...
    # [해설][흐름] 9) 재시도: `[retries]`를 한 번 읽어 예산(--max-retries > provider > global > 5)과 SDK 재시도 끄기 kwarg를 함께 결정. 경고는 여기서 로그.
    # [해설][주의] SDK 재시도 끄기는 extra_kwargs 병합 "이후"에 적용되므로 사용자가 `--model-params`로 max_retries를 줘도 무시된다(의도된 설계).
    ...
    # [해설] Claude-on-Vertex project/location 보정(location 없으면 여기서 실패).
    _apply_google_anthropic_vertex_kwargs(provider, kwargs)

    # [해설][흐름] 10) 생성 분기: openai_codex(ChatGPT OAuth 토큰 제공자) / config.toml class_path / 기본 `init_chat_model`.
    ...
    if provider == CODEX_PROVIDER:
        ...
        # [해설][주의] codex는 정적 API 키를 절대 쓰지 않는다(OAuth 토큰만). 토큰 파일 없음/만료는 모두 `/auth` 재로그인 안내로 수렴.
    ...
    # [해설][흐름] 11) 후처리: provider가 비어 있었다면 init_chat_model이 추론한 값 사용 → 비용 추적용 모델 메타데이터 부착(`cost_tracking._set_configured_model_metadata`).
    ...
    # [해설] profile override 체인: 모델 기본 profile < config.toml profile < CLI `--profile-override`(실패 시 예외).
    ...
    # [해설] 재시도 예산을 모델 객체에 스탬프(`MODEL_RETRIES_ATTR`). Pydantic 모델은 일반 setattr로 미정의 필드를 거부하므로 `object.__setattr__` 사용.
    # [해설] `model_retry.CodeModelRetryMiddleware`가 요청마다 이 값을 읽는다(analysis/03).
    ...
    # [해설] profile에서 컨텍스트 한도(`max_input_tokens`)와 명시적으로 False인 입력 모달리티를 추출(UI 경고·요약 임계값·첨부 검사에 사용, 추정).
...
# [해설][흐름] 모델 profile로 필수 능력 검사: profile 없음 → 안내만, `tool_calling=False` → 에러 출력 후 `sys.exit(1)`, 컨텍스트 < 8000 토큰 → 경고.
# [해설][주의] 라이브러리 함수인데 프로세스를 종료시킨다 — CLI 시작 경로 전용으로 쓰여야 한다 (추정: main.py에서 호출).
...
# [해설] 아래 3개는 lazy 프로세스 싱글턴 저장소. 모듈 하단 `_LazyProxy` 전역(credentials/runtime_state/console)이 첫 접근 시 채운다.
...
# [해설] Rich `Console` 싱글턴(double-checked locking). `rich` import를 실제 출력 시점까지 미룬다.
...
# [해설][흐름] 전역 `credentials`의 실제 생성 함수: `_ensure_bootstrap()`(dotenv 등) → `Credentials.from_environment(start_path=부트스트랩 시 캡처한 경로)`.
# [해설] 즉 "config.credentials에 처음 접근하는 순간"이 dotenv 로딩 시점이다(import 시점 아님). 실패 시 로그 후 재raise.
...
# [해설] `RuntimeState` 싱글턴 생성(double-checked locking).
...
# [해설][설계] 모듈 전역을 import 시점에 만들지 않고 첫 속성 접근 때 factory를 호출하는 프록시. `deepagents --help` 같은 경로에서 디스크 I/O·dotenv 로딩을 피한다.
# [해설] setattr/delattr도 싱글턴으로 전달 — 테스트의 `monkeypatch.setattr(config.credentials, ...)` 정리를 지원한다(추정: `__delattr__` docstring의 "patch cleanup").
class _LazyProxy:
...
# [해설] 공개 lazy 전역: `from deepagents_code.config import credentials, runtime_state, console`로 다른 모듈들이 사용한다.
# [해설][주의] 타입은 cast일 뿐 실제 객체는 `_LazyProxy`이므로 `isinstance(credentials, Credentials)`는 False이다.
