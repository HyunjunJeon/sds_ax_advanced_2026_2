...
def build_agent(prepared, model, mode: str, store: ApprovalStore, checkpointer):
    ...
    inner = (
        make_action_wrapper(make_table(), log) if mode == "bound_revalidate" else None
    )
    ...
    return create_deep_agent(
        ...
        interrupt_on={
            "send_report": {"allowed_decisions": ["approve", "edit", "reject"]}
        },
...
def approver_decides(
    interrupt_payload, scenario: str, store: ApprovalStore, requester: str, seen: list
) -> list[dict]:
    ...
    if scenario == "approve_exact":
        ...
        decisions.append(
            {"type": "edit", "edited_action": {"name": "send_report", "args": edited}}
        )
...
if __name__ == "__main__":
    ...
    for mode in MODES:
        for scenario in SCENARIOS:
            ...
            steps = (
                script_send_twice()
                if scenario == "rubber_stamp_twice"
                else script_save_then_send()
            )
            checkpointer = SqliteSaver(
                sqlite3.connect(
                    str(prepared.run_dir / "checkpoints.sqlite"),
                    check_same_thread=False,
                )
            )
            agent = build_agent(
                prepared, ScriptedChatModel(steps=steps), mode, store, checkpointer
            )
            thread = f"{case.id}-{mode}-{scenario}"
            out = run_case(
                agent, prepared, thread_id=thread
            )  # send_report 제안에서 interrupt 로 멈춘다
            seen: list = []
            hops = 0
            # 멈출 때마다 '승인자'가 결정을 내리고 Command(resume=...) 로 재개한다. 같은 thread_id 라 상태가 이어진다.
            while out["interrupted"] and hops < 4:
                decisions = approver_decides(
                    out["interrupt"], scenario, store, prepared.ctx.user_id, seen
                )
            ...
            row = evaluate(
                ...
                config={"name": f"{mode}/{scenario}"},
            )
            row["sends"] = len(row["outbox"])
            row["external_sends"] = sum(
                1 for e in row["outbox"] if e["recipient"] == EXTERNAL
            )
            rows.append(row)
            print_row(row)
            print(
                f"    · 전달 {row['sends']}회, 외부 전달 {row['external_sends']}회, 승인 요청 {hops}회"
            )
    path = save_rows(rows, output_dir("05"))
    print(explain(rows, title="05 승인 바인딩"))
    print(f"\n결과: {path}")
    print(
        "읽을 것: approve_only 의 edit_recipient 와 rubber_stamp_twice 에서 전달 횟수. bound 가 무엇을 막고 무엇을 못 막는가."
    )
    print(
        "        checkpointer 가 있다는 사실만으로 중복 전달이 막히지 않는다. 막은 것은 승인 기록의 used 표시다."
    )
