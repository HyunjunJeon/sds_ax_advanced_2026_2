...
DOCS = [
    ...
    "/projects/alpha/customer_feedback.md",
    "/projects/beta/hr_data.md",
]
MIN_SCORE = 0.5
RULES = [
...
LINTER_SAMPLES = {
    "ko: meeting_send_outside.md": (
        DATA / "attacks" / "meeting_send_outside.md"
    ...
    "ko: tests_omit_failures.md": (
        DATA / "attacks" / "tests_omit_failures.md"
    ).read_text(encoding="utf-8"),
    ...
    "ko: issues.md (정상)": (
        DATA / "base" / "projects" / "alpha" / "issues.md"
    ).read_text(encoding="utf-8"),
...
def part_a() -> list[dict]:
    ...
    """
    forbidden = (
        load_cases()[0].forbidden_strings
    )  # 사례 공통의 공개 금지 문자열 (전화·이메일·주민번호·카드 뒷자리)
    regex, lfm = PIIGuard("regex"), PIIGuard("lfm")  # 같은 Span 계약을 쓰는 두 탐지기
    policy = DisclosurePolicy(
        min_score=MIN_SCORE
    )  # 검토자에게 이름은 허용, 연락처·식별번호는 마스킹
    rows = []
    ...
    for vpath in DOCS:
        ...
        for name, guard in (("regex", regex), ("lfm", lfm)):
            ...
            masked, _ = mask_for_audience(text, raw, "internal_reviewer", policy)
            left = [s for s in forbidden if s in masked]
            labels = sorted({s.label for s in merged})
            print(
                f"  {name:<6} 원구간 {len(raw):>2} → 병합 {len(merged):>2} | 라벨 {labels}"
            )
...
def part_b() -> list[dict]:
    ...
    rows = []
    header = "  {:<34}".format("문서") + "".join(
        f"  R{i}   " for i in range(1, len(RULES) + 1)
    )
    print(header)
    for name, text in LINTER_SAMPLES.items():
        hits = linter.lint(text)
        cells = "".join(
            f"  {h.max_prob:.2f}{'*' if h.max_prob >= 0.5 else ' '}" for h in hits
        )
        print(f"  {name:<34}{cells}")
        rows.append(
            {
                "sample": name,
                "scores": {f"R{i + 1}": h.max_prob for i, h in enumerate(hits)},
...
if __name__ == "__main__":
    ...
    (out / "pii_spans.json").write_text(
        json.dumps(a, ensure_ascii=False, indent=1), encoding="utf-8"
    )
    (out / "policy_linter.json").write_text(
        json.dumps(b, ensure_ascii=False, indent=1), encoding="utf-8"
    )
    print(f"\n결과: {out}")
    print(
        "읽을 것: A 에서 병합 전후 구간 수와 남은 공개 금지 문자열. B 에서 같은 공격의 ko/en 점수 차이와 정상 문서의 점수."
    )
    print(
        "        탐지기가 놓친 것은 02·03 의 코드 기반 경계(권한 검사·공개 금지 목록)가 맡는다."
    )
