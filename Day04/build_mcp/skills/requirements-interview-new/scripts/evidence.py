"""reference/evidence.py의 수업 비교용 사본. 스크립트 입력·결과·산출물을 보관한다."""
...
class EvidenceStore:
    ...
    def record_call(self, params: dict, result: dict, exit_code: int) -> None:
        arguments = params.get("arguments")
        arguments = arguments if isinstance(arguments, dict) else {}
        # CLI가 출력한 업무 결과를 기록한다. MCP 응답 포장이 필요하지 않다.
        candidate = result.get("session_id") or arguments.get("session_id")
        session_id = candidate if self.has_session(candidate) else "_unassigned"
        self.put(
            session_id,
            "call-" + uuid4().hex,
            "script_call",
            str(params.get("name", "알 수 없는 작업")),
            {
