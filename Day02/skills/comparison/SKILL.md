---
name: comparison
description: 문서·조건·구간 사이의 차이를 비교표로 답할 때 사용한다.
---

# comparison

비교할 대상과 기준을 명확히 한다. 비교표의 각 항목은 label에 비교 대상/기준을, text에 차이를 쓴다. 다른 고객의 자료는 검색 범위를 넓혀 가져오지 않는다.
...
4. `AnswerPayload`를 반환한다. skill은 `comparison`이다. sections는 output.json의 각 heading과 items로 구성한다. 각 item에는 label, text, evidence_ids를 넣는다. 표가 아닌 경우 label은 빈 문자열이어도 된다.
