---
name: grounded-qa
description: 문서에 대한 일반 질문을 결론과 설명으로 답할 때 사용한다.
---

# grounded-qa

질문에 직접 답하는 결론을 먼저 쓰고, 적용 조건과 예외를 설명한다. 동일 사실의 장황한 반복을 피한다.
...
4. `AnswerPayload`를 반환한다. skill은 `grounded-qa`이다. sections는 output.json의 각 heading과 items로 구성한다. 각 item에는 label, text, evidence_ids를 넣는다. 표가 아닌 경우 label은 빈 문자열이어도 된다.
