---
name: procedure
description: 신청·처리·운영 방법을 단계별 절차로 답할 때 사용한다.
---

# procedure

대상과 적용 시점을 먼저 밝힌다. 제출 자료·기한·순서를 근거가 허용하는 만큼 설명한다. 추가 약정이 일반 규정보다 우선하는지 확인한다.
...
4. `AnswerPayload`를 반환한다. skill은 `procedure`이다. sections는 output.json의 각 heading과 items로 구성한다. 각 item에는 label, text, evidence_ids를 넣는다. 표가 아닌 경우 label은 빈 문자열이어도 된다.
